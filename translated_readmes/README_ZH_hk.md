...
安裝 Node.js 20 或更高版本後，運行：

```bash
npx -y ipollowork
```

該命令會自動識別目前的作業系統和 CPU，下載對應的 iPolloWork 正式桌面安裝包並啟動。需要指定版本時運行：
...
正式安裝包發佈在 [GitHub Releases](https://github.com/Devin-AXIS/iPolloWork/releases)。如果希望手動下載，請同時根據作業系統和 CPU 選擇文件：
