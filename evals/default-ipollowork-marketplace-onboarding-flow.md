# Default iPolloWork Marketplace onboarding flow
...
Marketplace containing built-in iPolloWork extension capabilities.

## Acceptance target

- Den download/sign-in copy does not claim that Den creates or loads a desktop workspace.
- Den introduces the built-in iPolloWork Marketplace and explains it appears after desktop sign-in.
- A signed-in desktop user can open Settings -> Marketplace and see `iPolloWork Marketplace` from Den.
- `iPolloWork Marketplace` includes built-in extension entries such as `iPolloWork Browser`, `Computer Use`, `OpenAI Image Gen`, `Google Workspace`, and `Ollama`.
- Built-in iPolloWork entries are rendered as cloud Marketplace entries with a `Built-in` status, not as locally injected desktop Marketplace rows.
- Signed-out desktop users can still use iPolloWork, but Marketplace nudges sign-in for built-in extensions and organization marketplaces.
...
8. Validate dev auth before UI recording: direct Den API sign-in with the seeded user should return `200` when `IPOLLOWORK_DEV_MODE=1`.
...
- Page includes `built-in iPolloWork Marketplace` or `built-in Marketplace`.
...
**Goal:** iPolloWork remains usable without account, while Marketplace clearly asks
for Cloud sign-in.
...
- The notice says the user can use iPolloWork without an account.
...
**Goal:** Desktop sign-in causes Den to provision and return the default iPolloWork
Marketplace for any user/org.

Steps:

1. Sign Electron into iPolloWork Cloud using the Daytona Den handoff flow.
2. Create or open a local workspace.
3. Open Settings -> Marketplace.
4. Click `Refresh` if the Marketplace list has not loaded yet.
5. Open `Filters` and inspect marketplace options.
6. Search for `iPolloWork Browser`.
7. Open the `iPolloWork Browser` card.

Expected outcome:

- `iPolloWork Marketplace` appears as a marketplace option.
- `iPolloWork Browser` appears as a Marketplace card from Den.
- The card shows `Built-in` or an equivalent built-in/ready status.
- The detail modal shows iPolloWork Browser setup/resource details from the Den extension manifest.
- The detail modal does not offer `Add` or `Remove` for built-in iPolloWork entries.
...
2. Den explains in plain language: Marketplaces contain plugins; iPolloWork Marketplace is included; org marketplaces appear after desktop sign-in.
...
6. Marketplace is pre-populated without manual refresh, with `iPolloWork Marketplace` grouped separately from org marketplaces.
...
stores and returns iPolloWork extension manifests; the desktop still owns local
