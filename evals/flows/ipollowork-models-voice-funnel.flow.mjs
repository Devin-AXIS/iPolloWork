...
const EMAIL_DOMAIN = "voice-eval.ipollowork.test";
const PASSWORD = "iPolloWorkVoiceEval123!";
const MOCK_INFERENCE_KEY = "ow_inf_voice_funnel";
const DEFAULT_STRIPE_WEBHOOK_SECRET = "whsec_ipollowork_eval";
const DEFAULT_STRIPE_PRICE_ID = "price_ipollowork_models_eval";
const VOICE_TRANSCRIPT_TEXT = "What can you help me with?";

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

function apiBase(ctx) {
  return ctx.env.IPOLLOWORK_EVAL_DEN_API_URL.trim().replace(/\/+$/, "");
}

function jsonHeaders(token) {
  return {
    "content-type": "application/json",
    origin: process.env.IPOLLOWORK_EVAL_DEN_ORIGIN?.trim() || process.env.IPOLLOWORK_EVAL_DEN_API_URL?.trim()?.replace(/\/+$/, "") || "http://localhost:8790",
    ...
    data: {
      object: {
        ...
        metadata: {
          ...
          ipollowork_product: "ipollowork_models",
...
export default {
  id: "ipollowork-models-voice-funnel",
  title: "Sign up, pay for iPolloWork Models, and start managed Voice Mode",
  spec: "evals/onboarding-welcome-flows.md#flow-28--ipollowork-models-path-explains-payment-before-value",
  requiredEnv: ["IPOLLOWORK_EVAL_DEN_API_URL"],
  steps: [
    {
      name: "Start mock iPolloWork Models voice broker",
      run: async (ctx) => {
        ctx.mockBroker = await startMockBroker();
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice broker fixture is listening for authenticated iPolloWork Models session requests.",
    ...
    {
      name: "Record paid iPolloWork Models subscription",
      run: async (ctx) => {
        const secret = ctx.env.IPOLLOWORK_EVAL_STRIPE_WEBHOOK_SECRET?.trim() || DEFAULT_STRIPE_WEBHOOK_SECRET;
        const priceId = ctx.env.IPOLLOWORK_EVAL_STRIPE_INFERENCE_PRICE_ID?.trim() || DEFAULT_STRIPE_PRICE_ID;
        ...
        ctx.assert(billing.json?.billing?.stripe?.hasActiveSubscription === true, "Billing status did not show an active iPolloWork Models subscription.");
        ctx.recordEvidence({ type: "assertion", status: "passed", assertion: "The paid iPolloWork Models subscription boundary is active for the new organization.", actual: billing.json.billing.stripe.subscription });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`(() => {
          localStorage.setItem("ipollowork.den.baseUrl", ${JSON.stringify(apiBase(ctx))});
          localStorage.setItem("ipollowork.den.apiBaseUrl", ${JSON.stringify(apiBase(ctx))});
          for (const key of ["ipollowork.den.authToken", "ipollowork.den.activeOrgId", "ipollowork.den.activeOrgSlug", "ipollowork.den.activeOrgName"]) localStorage.removeItem(key);
          window.location.hash = "#/settings/cloud-account";
          window.location.reload();
          return true;
        })()`);
        await ctx.waitFor("Boolean(window.__ipolloworkControl)", { timeoutMs: 60_000, label: "iPolloWork control API after auth reset" });

        const handoff = await denRequest(ctx, "/v1/auth/desktop-handoff", {
          method: "POST",
          token: ctx.den.token,
          body: { desktopScheme: "ipollowork" },
        });
        ctx.assert(handoff.response.ok, `Desktop handoff creation failed: ${handoff.response.status} ${handoff.text.slice(0, 300)}`);
        ctx.assert(typeof handoff.json?.ipolloworkUrl === "string", "Desktop handoff did not return ipolloworkUrl.");
        ...
        if (!(await ctx.hasText("Sign out"))) {
          ...
          await ctx.fill("#den-signin-link", handoff.json.ipolloworkUrl);
          await ctx.clickText("Finish sign-in", { timeoutMs: 15_000 });
        }
        await ctx.waitFor(`localStorage.getItem("ipollowork.den.authToken") === ${JSON.stringify(ctx.den.token)}`, {
          timeoutMs: 45_000,
        ...
        await ctx.eval(`(() => {
          ...
          localStorage.setItem("ipollowork.preferences", JSON.stringify({ ...prefs, hasCompletedOnboarding: true }));
          window.location.hash = "#/settings/cloud-account";
          return true;
        })()`);
        await ctx.expectText("Sign out", { timeoutMs: 30_000 });
        await ctx.screenshot("paid-den-account-signed-in", { claim: "Desktop is signed in to the newly paid iPolloWork Models account.", requireText: ["Sign out"] });
      },
    },
    {
      name: "Create workspace via UI",
      run: async (ctx) => {
        const workspaceDir = ctx.env.IPOLLOWORK_EVAL_WORKSPACE_DIR?.trim() || join(tmpdir(), `ipollowork-models-voice-${Date.now()}`);
        ...
        await ctx.eval(`(() => {
          ...
          if (nameInput) {
            ...
            setter.call(nameInput, "iPolloWork Models Voice Eval");
        ...
        await ctx.eval(`(() => {
          localStorage.setItem("ipollowork.extension.enabled.ipollowork-voice", "1");
          window.dispatchEvent(new CustomEvent("ipollowork:extension-state-changed", { detail: { id: "ipollowork-voice", enabled: true } }));
          return true;
        })()`);

        await ctx.expectText("iPolloWork Models", { timeoutMs: 30_000 });
        await ctx.screenshot("workspace-created-via-ui", { claim: "Workspace was created through the Add workspace UI flow.", requireText: ["iPolloWork Models"] });
      },
    },
    {
      ...
      run: async (ctx) => {
        const serverConfig = await ctx.eval(`(() => ({
          baseUrl: localStorage.getItem("ipollowork.server.urlOverride") || localStorage.getItem("ipollowork.server.active") || "",
          hostToken: localStorage.getItem("ipollowork.server.hostToken") || "",
        }))()`);
        ...
        for (const [key, value] of [["IPOLLOWORK_API_KEY", MOCK_INFERENCE_KEY], ["IPOLLOWORK_INFERENCE_BASE_URL", ctx.mockBroker.baseUrl]]) {
          ...
          if (!savedViaUI) {
            ...
            const envResponse = await fetch(`${serverConfig.baseUrl}/env`, {
              method: "PUT",
              headers: { "x-ipollowork-host-token": serverConfig.hostToken, "content-type": "application/json" },
            ...
            await ctx.waitFor("Boolean(window.__ipolloworkControl)", { timeoutMs: 30_000, label: "control API after env reload" });
            await ctx.clickText("Environment", { timeoutMs: 10_000 });
            await sleep(1_000);
          }
          ctx.log(`Saved ${key}`);
        ...
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice credentials (IPOLLOWORK_API_KEY + IPOLLOWORK_INFERENCE_BASE_URL) were saved through the Settings > Environment UI.",
          actual: ["IPOLLOWORK_API_KEY", "IPOLLOWORK_INFERENCE_BASE_URL"],
        });
        await ctx.screenshot("managed-voice-env-configured-via-ui", {
          ...
          requireText: ["IPOLLOWORK_API_KEY", "IPOLLOWORK_INFERENCE_BASE_URL"],
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("window.__ipolloworkControl.listActions().some((action) => action.id === 'voice.panel.open' && !action.disabled)", {
          timeoutMs: 30_000,
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        ctx.assert(request.authorization === `Bearer ${MOCK_INFERENCE_KEY}`, "Voice Mode did not authenticate to the iPolloWork Models broker with the paid inference key.");
        ctx.assert(request.url === "/voice/realtime/session", "Voice Mode did not call the managed realtime session endpoint.");
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Voice Mode requested a managed iPolloWork Models realtime session.",
        ...
        ctx.assert(
          ...
          "Voice Mode showed a direct OpenAI API key missing error instead of using the managed iPolloWork Models path.",
        );
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
        ...
        await ctx.screenshot("managed-voice-mode-started", {
          claim: "Voice Mode started through the managed iPolloWork Models session path via UI click.",
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const statusResult = await ctx.eval(`window.__ipolloworkControl.execute("voice.status")`);
        ...
        await ctx.screenshot("voice-mode-active-with-transcript", {
          claim: "Voice Mode is active through the managed iPolloWork Models path, with timeline and connection state visible.",
          requireText: ["Voice Mode"],
          rejectText: ["OpenAI API key missing"],
        });
