...
const vo = await loadVoiceoverParagraphs("docs-ipollowalk-connect");
...
async function waitForInstaller(ctx) {
  await ctx.waitFor(
    ...
    { timeoutMs: 30_000, label: "iPolloWalk Connect installer" },
  );
...
async function grantDocsClipboardPermissions(ctx) {
  ...
  await ctx.client.send("Browser.grantPermissions", {
    origin: new URL(baseUrl("IPOLLOWALK_EVAL_DOCS_URL")).origin,
    permissions: ["clipboardReadWrite", "clipboardSanitizedWrite"],
  });
}

export default {
  id: "docs-ipollowalk-connect",
  title: "Use the iPolloWalk Connect installer from the docs or landing page",
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: ["IPOLLOWALK_EVAL_DOCS_URL", "IPOLLOWALK_EVAL_LANDING_URL"],
  steps: [
    {
      name: "Frame 1",
      run: async (ctx) => {
        await ctx.prove("The iPolloWalk Connect docs put the complete client installer at the top of the page.", {
          voiceover: vo[0],
          action: async () => {
            await navigate(ctx, `${baseUrl("IPOLLOWALK_EVAL_DOCS_URL")}/cloud/run-in-the-cloud/cloud-mcp`);
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("The direct OpenCode docs link opens the installer with OpenCode selected.", {
          voiceover: vo[3],
          action: async () => {
            await navigate(ctx, `${baseUrl("IPOLLOWALK_EVAL_DOCS_URL")}/cloud/run-in-the-cloud/cloud-mcp#connect-mcp-install-opencode`);
            await waitForInstaller(ctx);
          },
          assert: async () => {
            const actual = await ctx.eval(`(() => {
    ...
    },
    {
      name: "Frame 5",
      run: async (ctx) => {
        await ctx.prove("The landing page keeps the same iPolloWalk Connect installer, including Codex and ChatGPT Desktop.", {
          voiceover: vo[4],
          action: async () => {
            await navigate(ctx, `${baseUrl("IPOLLOWALK_EVAL_LANDING_URL")}/#connect-mcp`);
            await waitForInstaller(ctx);
          },
          assert: async () => {
            const actual = await ctx.eval(`(() => {
