...
const EMAIL_DOMAIN = "voice-eval.ipollowalk.test";
const PASSWORD = "iPolloWalkVoiceEval123!";
const MOCK_INFERENCE_KEY = "ow_inf_voice_funnel";
const DEFAULT_STRIPE_WEBHOOK_SECRET = "whsec_ipollowalk_eval";
const DEFAULT_STRIPE_PRICE_ID = "price_ipollowalk_models_eval";
const VOICE_TRANSCRIPT_TEXT = "What can you help me with?";

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

function apiBase(ctx) {
  return ctx.env.IPOLLOWALK_EVAL_DEN_API_URL.trim().replace(/\/+$/, "");
}

function jsonHeaders(token) {
  return {
    "content-type": "application/json",
    origin: process.env.IPOLLOWALK_EVAL_DEN_ORIGIN?.trim() || process.env.IPOLLOWALK_EVAL_DEN_API_URL?.trim()?.replace(/\/+$/, "") || "http://localhost:8790",
    ...
    data: {
      object: {
        ...
        metadata: {
          ...
          ipollowalk_product: "ipollowalk_models",
...
export default {
  id: "ipollowalk-models-voice-funnel",
  title: "Sign up, pay for iPolloWalk Models, and start managed Voice Mode",
  spec: "evals/onboarding-welcome-flows.md#flow-28--ipollowalk-models-path-explains-payment-before-value",
  requiredEnv: ["IPOLLOWALK_EVAL_DEN_API_URL"],
  steps: [
    {
      name: "Start mock iPolloWalk Models voice broker",
      run: async (ctx) => {
        ctx.mockBroker = await startMockBroker();
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice broker fixture is listening for authenticated iPolloWalk Models session requests.",
    ...
    {
      name: "Record paid iPolloWalk Models subscription",
      run: async (ctx) => {
        const secret = ctx.env.IPOLLOWALK_EVAL_STRIPE_WEBHOOK_SECRET?.trim() || DEFAULT_STRIPE_WEBHOOK_SECRET;
        const priceId = ctx.env.IPOLLOWALK_EVAL_STRIPE_INFERENCE_PRICE_ID?.trim() || DEFAULT_STRIPE_PRICE_ID;
        ...
        ctx.assert(billing.json?.billing?.stripe?.hasActiveSubscription === true, "Billing status did not show an active iPolloWalk Models subscription.");
        ctx.recordEvidence({ type: "assertion", status: "passed", assertion: "The paid iPolloWalk Models subscription boundary is active for the new organization.", actual: billing.json.billing.stripe.subscription });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.eval(`(() => {
          localStorage.setItem("ipollowalk.den.baseUrl", ${JSON.stringify(apiBase(ctx))});
          localStorage.setItem("ipollowalk.den.apiBaseUrl", ${JSON.stringify(apiBase(ctx))});
          for (const key of ["ipollowalk.den.authToken", "ipollowalk.den.activeOrgId", "ipollowalk.den.activeOrgSlug", "ipollowalk.den.activeOrgName"]) localStorage.removeItem(key);
          window.location.hash = "#/settings/cloud-account";
          window.location.reload();
          return true;
        })()`);
        await ctx.waitFor("Boolean(window.__ipollowalkControl)", { timeoutMs: 60_000, label: "iPolloWalk control API after auth reset" });

        const handoff = await denRequest(ctx, "/v1/auth/desktop-handoff", {
          method: "POST",
          token: ctx.den.token,
          body: { desktopScheme: "ipollowalk" },
        });
        ctx.assert(handoff.response.ok, `Desktop handoff creation failed: ${handoff.response.status} ${handoff.text.slice(0, 300)}`);
        ctx.assert(typeof handoff.json?.ipollowalkUrl === "string", "Desktop handoff did not return ipollowalkUrl.");
        ...
        if (!(await ctx.hasText("Sign out"))) {
          ...
          await ctx.fill("#den-signin-link", handoff.json.ipollowalkUrl);
          await ctx.clickText("Finish sign-in", { timeoutMs: 15_000 });
        }
        await ctx.waitFor(`localStorage.getItem("ipollowalk.den.authToken") === ${JSON.stringify(ctx.den.token)}`, {
          timeoutMs: 45_000,
        ...
        await ctx.eval(`(() => {
          ...
          localStorage.setItem("ipollowalk.preferences", JSON.stringify({ ...prefs, hasCompletedOnboarding: true }));
          window.location.hash = "#/settings/cloud-account";
          return true;
        })()`);
        await ctx.expectText("Sign out", { timeoutMs: 30_000 });
        await ctx.screenshot("paid-den-account-signed-in", { claim: "Desktop is signed in to the newly paid iPolloWalk Models account.", requireText: ["Sign out"] });
      },
    },
    {
      name: "Create workspace via UI",
      run: async (ctx) => {
        const workspaceDir = ctx.env.IPOLLOWALK_EVAL_WORKSPACE_DIR?.trim() || join(tmpdir(), `ipollowalk-models-voice-${Date.now()}`);
        ...
        await ctx.eval(`(() => {
          ...
          if (nameInput) {
            ...
            setter.call(nameInput, "iPolloWalk Models Voice Eval");
        ...
        await ctx.eval(`(() => {
          localStorage.setItem("ipollowalk.extension.enabled.ipollowalk-voice", "1");
          window.dispatchEvent(new CustomEvent("ipollowalk:extension-state-changed", { detail: { id: "ipollowalk-voice", enabled: true } }));
          return true;
        })()`);

        await ctx.expectText("iPolloWalk Models", { timeoutMs: 30_000 });
        await ctx.screenshot("workspace-created-via-ui", { claim: "Workspace was created through the Add workspace UI flow.", requireText: ["iPolloWalk Models"] });
      },
    },
    {
      ...
      run: async (ctx) => {
        const serverConfig = await ctx.eval(`(() => ({
          baseUrl: localStorage.getItem("ipollowalk.server.urlOverride") || localStorage.getItem("ipollowalk.server.active") || "",
          hostToken: localStorage.getItem("ipollowalk.server.hostToken") || "",
        }))()`);
        ...
        for (const [key, value] of [["IPOLLOWALK_API_KEY", MOCK_INFERENCE_KEY], ["IPOLLOWALK_INFERENCE_BASE_URL", ctx.mockBroker.baseUrl]]) {
          ...
          if (!savedViaUI) {
            ...
            const envResponse = await fetch(`${serverConfig.baseUrl}/env`, {
              method: "PUT",
              headers: { "x-ipollowalk-host-token": serverConfig.hostToken, "content-type": "application/json" },
            ...
            await ctx.waitFor("Boolean(window.__ipollowalkControl)", { timeoutMs: 30_000, label: "control API after env reload" });
            await ctx.clickText("Environment", { timeoutMs: 10_000 });
            await sleep(1_000);
          }
          ctx.log(`Saved ${key}`);
        ...
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Managed voice credentials (IPOLLOWALK_API_KEY + IPOLLOWALK_INFERENCE_BASE_URL) were saved through the Settings > Environment UI.",
          actual: ["IPOLLOWALK_API_KEY", "IPOLLOWALK_INFERENCE_BASE_URL"],
        });
        await ctx.screenshot("managed-voice-env-configured-via-ui", {
          ...
          requireText: ["IPOLLOWALK_API_KEY", "IPOLLOWALK_INFERENCE_BASE_URL"],
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.waitFor("window.__ipollowalkControl.listActions().some((action) => action.id === 'session.create_task' && !action.disabled)", {
          timeoutMs: 60_000,
        ...
        await ctx.waitFor("window.__ipollowalkControl.listActions().some((action) => action.id === 'voice.panel.open' && !action.disabled)", {
          timeoutMs: 30_000,
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        ctx.assert(request.authorization === `Bearer ${MOCK_INFERENCE_KEY}`, "Voice Mode did not authenticate to the iPolloWalk Models broker with the paid inference key.");
        ctx.assert(request.url === "/voice/realtime/session", "Voice Mode did not call the managed realtime session endpoint.");
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
          assertion: "Voice Mode requested a managed iPolloWalk Models realtime session.",
        ...
        ctx.assert(
          ...
          "Voice Mode showed a direct OpenAI API key missing error instead of using the managed iPolloWalk Models path.",
        );
        ctx.recordEvidence({
          type: "assertion",
          status: "passed",
        ...
        await ctx.screenshot("managed-voice-mode-started", {
          claim: "Voice Mode started through the managed iPolloWalk Models session path via UI click.",
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        const statusResult = await ctx.eval(`window.__ipollowalkControl.execute("voice.status")`);
        ...
        await ctx.screenshot("voice-mode-active-with-transcript", {
          claim: "Voice Mode is active through the managed iPolloWalk Models path, with timeline and connection state visible.",
          requireText: ["Voice Mode"],
          rejectText: ["OpenAI API key missing"],
        });
