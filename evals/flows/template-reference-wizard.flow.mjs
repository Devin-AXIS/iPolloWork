...
export default {
  ...
  title: "Template references: skip, parse, review, edit and submit",
  kind: "user-facing",
  ...
  steps: [{ name: "Two-step reference workflow", run: async (ctx) => {
    ...
    await ctx.clickText("返回", { selector: `${dialog} button` });
    ...
    await ctx.waitForText("解析已结束");
    await ctx.prove("Parsed content can be reviewed and downloaded as JSON", {
      voiceover: "解析完成后可以查看提取内容、下载统一的 JSON 结果，再继续填写信息。",
      action: () => ctx.eval(`document.querySelector('${dialog} details summary').click()`),
      assert: async () => { await ctx.waitForText("设计团队"); ctx.assert(await ctx.eval(`document.querySelector('${dialog} [role="progressbar"]').getAttribute('aria-valuenow') === '100'`), "Parsing reached 100%."); },
      screenshot: { name: "parsed-reference", requireText: ["下载解析结果（JSON）", "设计团队"] },
    });
