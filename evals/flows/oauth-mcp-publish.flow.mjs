 ...
 *   1. Signs in to iPolloWalk Cloud via desktop handoff and creates a workspace.
 ...
 * seeded demo org (alex@acme.test / iPolloWalkDemo123!).
 */
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Owner is signed in to iPolloWalk Cloud", {
          action: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        const wsPath = join(homedir(), ".ipollowalk", "two-electron-demo", "eval-workspace-a");
