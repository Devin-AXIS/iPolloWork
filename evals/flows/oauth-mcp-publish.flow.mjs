 ...
 *   1. Signs in to iPolloWork Cloud via desktop handoff and creates a workspace.
 ...
 * seeded demo org (alex@acme.test / iPolloWorkDemo123!).
 */
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Owner is signed in to iPolloWork Cloud", {
          action: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        const wsPath = join(homedir(), ".ipollowork", "two-electron-demo", "eval-workspace-a");
