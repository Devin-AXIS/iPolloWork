...
async function showWorkspaceSettings(ctx, panel) {
...
async function openMarketplace(ctx) {
  await ensureRenderedApp(ctx);
  await showWorkspaceSettings(ctx, "cloud-marketplaces");
...
async function openMcpSettings(ctx) {
  await ensureRenderedApp(ctx);
  await showWorkspaceSettings(ctx, "extensions/mcp");
      ...
      run: async (ctx) => {
        await ctx.prove("Persistent state shows skill installed and no broken MCP row", {
          ...
          action: async () => {
            ...
            await showWorkspaceSettings(ctx, "extensions/skills");
