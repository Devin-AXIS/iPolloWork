...
import { resolveInstalledPluginContributions } from "../../apps/app/src/react-app/plugin-ui/plugin-ui-contributions";
...
import { SidePanel } from "../../apps/app/src/react-app/domains/session/panel/side-panel";
import { usePanelTabStore } from "../../apps/app/src/react-app/domains/session/panel/panel-tab-store";
...
window.__proofPanels = usePanelTabStore;
window.__proofSetLocale = setLocale;
...
window.__proofImageSurface = resolveInstalledPluginContributions([{enabled:true,pluginId:setup.manifest.id,name:setup.manifest.name,disabledResourceIds:[],manifest:setup.manifest}]).workspaceApps.find(item=>item.pluginId==="image-studio");
const client = {
  baseUrl: endpoint,
  ...
  downloadWorkspaceFile: async (_id, path) => {
    if (window.__proofMediaDelay) await new Promise(resolve => setTimeout(resolve, window.__proofMediaDelay));
    return { data: await (await fetch(endpoint + "/raw?path=" + encodeURIComponent(path))).arrayBuffer() };
  },
...
if (setup.designPage) {
  ...
  store.clearSession("selection-proof");
  usePanelTabStore.setState({mediaEdits:[]});
  store.openTab("selection-proof", {id:"design-proof",type:"design",label:"Design",sessionId:"source-project",path:setup.designPage});
}
function Fixture() {
  const edits = usePanelTabStore(state => state.mediaEdits);
  const result = edits.filter(edit => edit.results.length).at(-1)?.resultPath;
  ...
  return <main style={{ height: "100vh", display: "flex", flexDirection: "column" }} className="bg-background text-foreground">
    <header className="border-b px-4 py-2 text-sm">视频图片联动验证 · 真实编辑界面与本地保存，AI 响应为模拟测试图
      ...
      {result ? <div data-testid="generated-result-card" style={{maxWidth:300}}><OpenTargetProvider openTargets={[{id:result,kind:"file",value:result,name:result.split("/").pop(),preview:"image",exists:true,confidence:1,reason:"fixture"}]} onOpenTarget={target=>usePanelTabStore.getState().openMediaEditResult("selection-proof","selection-proof",target.value,window.__proofImageSurface)}><ArtifactList messages={[]} supplementalFiles={[result]} /></OpenTargetProvider></div> : null}
    </header>
    <section style={{ position: "relative", flex: 1, minHeight: 0 }}>
      {surface === "design" ? <SidePanel client={client} workspaceId="selection-proof" workspaceRoot={setup.root} sessionId="selection-proof" onClose={()=>{}} onAskAi={()=>{}} /> : <iframe ref={ref} src={studioUrl} title="Video Studio" style={{ width: "100%", height: "100%", border: 0 }} />}
