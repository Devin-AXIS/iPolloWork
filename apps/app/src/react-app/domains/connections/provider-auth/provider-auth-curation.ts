...
export const formatProviderAuthName = (id: string, fallback?: string) => {
...
export const isOpenAiProvider = (id: string, fallbackName?: string) => {
  const normalizedId = id.trim().toLowerCase();
  const normalizedName = fallbackName?.trim().toLowerCase() ?? "";
  return normalizedId === "openai" || normalizedName === "openai";
};

export const isAnthropicProvider = (id: string, fallbackName?: string) => {
  const normalizedId = id.trim().toLowerCase();
  const normalizedName = fallbackName?.trim().toLowerCase() ?? "";
...
const getCollapsedGroup = (id: string) =>
  COLLAPSED_PROVIDER_GROUPS.find((group) => group.canonicalId === id);
