...
const SUPPORT_EMAIL = "team@ipollowalklabs.com";
const SUPPORT_MAILTO = `mailto:${SUPPORT_EMAIL}?subject=iPolloWalk%20Den%20remote%20worker%20upgrade`;

/**
 * Small inline link rendered inside the remote-worker error card. When clicked,
 * it opens a dialog explaining the iPolloWalk Den upgrade situation and how to
 * reach support.
 */
export function IPolloWalkDenHelpLink() {
  ...
  return (
    <>
      ...
      >
        Using iPolloWalk Den Remote Workers? Click here
      </button>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>iPolloWalk Den remote workers</DialogTitle>
            <DialogDescription>
              We recently upgraded our servers. If your remote worker was
              provisioned before that upgrade, it may no longer be compatible
              with the current iPolloWalk app.
            </DialogDescription>
          </DialogHeader>
