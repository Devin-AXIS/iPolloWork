...
export function videoIllustrationReferenceInstruction(reference: VideoIllustrationAiReference) {
  return [
    ...
    "- Include a valid doctype and a complete html/head/body document. Set body margin to 0, hide overflow, and render the complete illustration inside one root SVG with viewBox `0 0 1600 900`, width and height set to 100%, and preserveAspectRatio `xMidYMid meet`. Keep every visual element inside that viewBox so the full illustration remains visible at every size. This file itself is the final asset; do not create a PNG or merely display markup in chat.",
