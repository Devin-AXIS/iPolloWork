...
export function videoTaskSystemContext(
  ...
  const voiceoverContract = options.includeVoiceover ? [
    ...
    `- Build one ordered scene array from exact visible text, then make one media call with action \`speech_synthesize_workspace_batch\`, \`compositionPath: "${projectDirectory}/index.html"\`, the selected voice/model, and one immutable \`assets/voiceover-<revision>-<scene>.mp3\` output per scene. Text and sceneText must be identical.`,
