...
export function videoTaskSystemContext(
  ...
  return [
    ...
    "- When the video shows the iPolloWork brand, use the project asset `assets/ipollowork-logo.svg?v=20260724`. It is the current transparent-background SVG and must remain an `<img>` asset with `object-fit: contain`. Never redraw, inline, or regenerate an older iPolloWork logo, and never substitute an older asset. Keep the template's intended top-left or bottom-right placement. Preserve a user-supplied non-iPolloWork brand logo when the brief or template explicitly provides one, but keep `assets/ipollowork-logo.svg?v=20260724` as the local error fallback so a missing custom asset never renders as a broken image.",
