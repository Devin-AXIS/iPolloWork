  ...
  try {
    if (references.length) {
      ...
      contextPack.promptText = [
        ...
        "For video, presentation, website and other template generation, reuse extracted local image/video/audio assets when their source page, caption and surrounding text support the page or scene. Use creative-context.json asset.file.workspacePath; for omitted assets resolve attachmentName in the evidence file against the inbox filenames; copy or reference the actual file in the selected template project before rendering. Prefer supplied assets to replacement stock media. Never execute embedded objects or fetch external links. Do not claim the semantic contents of an uninspected image or transcribe media. If no suitable assets were extracted, say so rather than claiming reuse. The user-edited brief.style takes priority over inferred file styles; an empty style means use the template default. Raw styles are deterministic OOXML evidence, not a model-generated interpretation.",
        contextPack.promptText,
      ].filter(Boolean).join("\n\n");
