...
import type { TemplateCatalogItem, TemplateCategory, TemplateSessionSnapshot } from "@ipollowork/types/templates";
...
export function DesignTemplateDialog(props: DesignTemplateDialogProps) {
  ...
  const templatesQuery = useQuery({
    queryKey: ["design-studio-templates", props.workspaceId] as const,
    queryFn: () => {
  ...
  const loadCover = React.useCallback((templateId: string) => {
    if (!getCover) return Promise.reject(new Error(t("design_templates.unavailable")));
    return getCover(props.workspaceId, templateId);
  }, [getCover, props.workspaceId]);
            ...
            {templatesQuery.isLoading ? (
              ...
              <div className="grid grid-cols-1 gap-3 pb-1 sm:grid-cols-2 lg:grid-cols-3">
                {visible.map((template) => (
                  ...
                  >
                    <TemplateCover template={template} load={loadCover} />
