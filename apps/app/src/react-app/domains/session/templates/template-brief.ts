...
export function templateBriefPrompt(input: {
  ...
  if (input.template.id === "ipollowork.wechat-article") {
    return `${base} Fixed-brand exception: preserve every data-ipw-fixed="true" node, fixed-hero.jpg, fixed-footer-cta.jpg, locked brand colors, and fixed brand images. Apply brief.style only to editable non-fixed styling. Update article copy, non-fixed middle images, and the CTA href when provided.`;
  }
  const visualSystemInstruction = "User brief.style overrides editable styling, never fixed-brand nodes. When brief.style is non-empty, apply it consistently through design-tokens.css and --ipw-* tokens. Otherwise: " + "Keep design-tokens.css and preserve its current theme as the visual source of truth; do not change the managed theme block. Reuse layout and motion. Preserve editor/export/runtime hooks.";
