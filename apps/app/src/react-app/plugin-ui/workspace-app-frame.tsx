...
function WorkspaceAppInspector({ context, onClose, onCallTool, onOpenAuthorizations, onOptimizePrompt, composer = false }: WorkspaceAppInspectorProps) {
  ...
  const renderField = (field: PluginUiInspectorContextV1["fields"][number]) => (
    <div key={field.id} className={cn("min-w-0 flex flex-col gap-1", composer && (field.control === "image" ? "w-20 shrink-0 self-start" : field.control === "textarea" ? "min-w-0 flex-1 basis-40" : "w-40 max-w-full shrink-0"))} data-inspector-field={field.id}>
      <span className={cn("text-[10px] text-muted-foreground", composer && "text-xs", composer && (field.control === "textarea" || field.control === "image") && "sr-only")}>{field.label}</span>
      {field.control === "image" ? (
        <>
          <div className={cn("relative overflow-hidden rounded-xl border border-border bg-muted/30", composer && "rounded-[16px] border-0 bg-white shadow-none")}>
            <button type="button" aria-label={`${field.value ? "更换" : "上传"}${field.label}`}
              ...
              {uploadingField === field.id ? <span role="status" className="flex items-center gap-2 text-xs"><Loader2 className="size-4 animate-spin" />正在上传…</span>
                ...
                : <span className="flex flex-col items-center gap-2 text-muted-foreground"><ImagePlus className="size-6" />{composer ? <span className="text-[11px]">参考图</span> : <><span className="text-xs">点击上传{field.label}</span><span className="text-[10px]">PNG / JPG / WebP · 最大 20 MB</span></>}</span>}
            </button>
          ...
          {composer ? <input type="hidden" name={field.id} value={field.value} /> : <details className="text-[11px] text-muted-foreground">
        ...
        <Select key={`${field.id}:${field.value}:${JSON.stringify(field.options)}`} name={field.id}
          ...
          <SelectTrigger className={cn("w-full border-transparent bg-muted shadow-none hover:bg-muted/80", composer && "rounded-[8px] bg-[#f0f0f2] text-zinc-900 hover:bg-[#e6e6e9]")} aria-label={field.label}>
      ...
      <form ref={formRef} onInput={() => setHasPrompt(Boolean(formRef.current?.querySelector<HTMLTextAreaElement>('textarea[name="prompt"]')?.value.trim()))} className={composer ? "flex flex-wrap items-end gap-3" : "space-y-3"} onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        {composer ? <>
          <div className="flex w-full items-start gap-3">
            ...
            {context.fields.filter(field => !isAdvancedField(field) && field.control === "textarea").map(renderField)}
          </div>
        ...
        {composer && onOptimizePrompt ? <div className="ml-auto flex items-center gap-2">
...
export function WorkspaceAppFrame(props: WorkspaceAppFrameProps) {
  ...
  useEffect(() => {
    ...
    const receiveImageStudioReference = (event: MessageEvent) => {
      ...
      if (event.data.type === "ipollowork:image-studio:model-menu") {
  ...
  }, [navigate, props.client, props.workspaceId, props.onDisplayModeChange, props.sessionId, props.surface.pluginId, resource]);
  ...
  return (
    <div className={cn("relative flex h-full min-h-0 w-full overflow-hidden bg-background", inspectorBelowAppToolbar && inspectorOpen && inspectorContext && "flex-col bg-[#f4f4f2] dark:bg-[#151515]", props.className)}
              ...
              <WorkspaceAppInspector
                ...
                onOptimizePrompt={async args => {
                  const response = await props.client.callExtensionAction({
                    ...
                    args: { prompt: args.prompt, referencePath: args.referencePath },
      ...
      {modelMenu ? <DropdownMenu open onOpenChange={open => {
        if (!open) {
          setModelMenu(null);
          iframeRef.current?.contentWindow?.postMessage({ type: "ipollowork:image-studio:model-menu-closed" }, "*");
        }
      }}>
        <DropdownMenuTrigger aria-label="切换图片模型" style={{ position: "fixed", left: modelMenu.left, top: modelMenu.top, width: modelMenu.width, height: modelMenu.height, opacity: 0 }} />
        <DropdownMenuContent align="end" className="w-64">
          <DropdownMenuRadioGroup value={modelMenu.value} onValueChange={value => {
            iframeRef.current?.contentWindow?.postMessage({ type: "ipollowork:image-studio:select-model", value }, "*");
            setModelMenu(null);
          }}>
      ...
      <Dialog open={imagePreview !== null} onOpenChange={open => {
        if (open) return;
        iframeRef.current?.contentWindow?.postMessage({ type: "ipollowork:image-studio:preview-closed", path: imagePreview?.path }, "*");
        ...
        <DialogContent className="flex h-[min(80dvh,800px)] flex-col gap-4 sm:max-w-[min(90vw,1000px)]" data-testid="image-studio-preview" aria-describedby={undefined}>
          ...
          {imagePreview ? <ImagePreview src={imagePreview.src} alt={imagePreview.name} className="min-h-0 flex-1 rounded-lg" /> : null}
        </DialogContent>
      </Dialog>
    </div>
  );
