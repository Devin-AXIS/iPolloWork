...
export type iPolloWalkServerCapabilities = {
...
export type iPolloWalkServerStatus = "connected" | "disconnected" | "limited";

export type iPolloWalkServerDiagnostics = {
  ok: boolean;
  version: string;
  uptimeMs: number;
  readOnly: boolean;
  ...
  workspace: iPolloWalkWorkspaceInfo | null;
  authorizedRoots: string[];
  server: { host: string; port: number; configPath?: string | null };
  tokenSource: { client: string; host: string };
};

export type iPolloWalkRuntimeServiceName = "ipollowalk-server" | "opencode";

export type iPolloWalkRuntimeServiceSnapshot = {
  name: iPolloWalkRuntimeServiceName;
  enabled: boolean;
  running: boolean;
...
export type iPolloWalkRuntimeSnapshot = {
  ok: boolean;
  orchestrator?: {
    version: string;
    startedAt: number;
  ...
  upgrade?: {
    ...
    services: iPolloWalkRuntimeServiceName[];
  };
  services: iPolloWalkRuntimeServiceSnapshot[];
};

export type iPolloWalkServerSettings = {
  urlOverride?: string;
  portOverride?: number;
  token?: string;
  hostToken?: string;
...
export type iPolloWalkWorkspaceInfo = WorkspaceInfo;

export type iPolloWalkWorkspaceList = {
  items: iPolloWalkWorkspaceInfo[];
  workspaces?: WorkspaceInfo[];
  activeId?: string | null;
};

export type iPolloWalkSessionMessage = {
  info: Message;
  parts: Part[];
};

export type iPolloWalkSessionSnapshot = {
  session: Session;
  messages: iPolloWalkSessionMessage[];
  todos: Todo[];
  status:
    | { type: "idle" }
    | { type: "busy" }
...
export type iPolloWalkPluginItem = {
  spec: string;
...
export type iPolloWalkSkillItem = {
  name: string;
  path: string;
  description: string;
  scope: "project" | "global";
...
export type iPolloWalkSkillContent = {
  item: iPolloWalkSkillItem;
  content: string;
};

export type iPolloWalkHubSkillItem = {
  name: string;
  description: string;
  trigger?: string;
  source: {
...
export type iPolloWalkHubRepo = {
  owner?: string;
  repo?: string;
  ref?: string;
};

export type iPolloWalkWorkspaceFileContent = {
  path: string;
  content: string;
  bytes: number;
  updatedAt: number;
...
export type iPolloWalkWorkspaceFileWriteResult = {
  ok: boolean;
  path: string;
  bytes: number;
  updatedAt: number;
...
export type iPolloWalkWorkspaceCatalogEntry = {
  path: string;
  kind: "file" | "dir";
  size: number;
  mtimeMs: number;
...
export type iPolloWalkWorkspaceFileDeleteResult = {
  ok: boolean;
  path: string;
  code?: string;
};

export type iPolloWalkAuthorizedFoldersResponse = {
  folders: string[];
  hiddenCount: number;
  workspaceRoot: string;
};

export type iPolloWalkAuthorizedFoldersUpdateResponse = {
  folders: string[];
  hiddenCount: number;
  updatedAt: number;
};

export type iPolloWalkRuntimeConfigMigrationResult = {
  migrated: boolean;
  keys: string[];
  legacyKeys: string[];
...
export type iPolloWalkRuntimeConfigStatus = {
...
export type iPolloWalkDesktopCloudSyncChange = {
  id: string;
...
export type iPolloWalkDesktopCloudSyncState = {
  entries: Record<string, unknown>;
  updatedAt: number;
  version: 1;
};

export type iPolloWalkDesktopCloudSyncResult = {
  changes: iPolloWalkDesktopCloudSyncChange[];
  state: iPolloWalkDesktopCloudSyncState;
};

export type iPolloWalkCloudPluginInstallResult = {
  item: CloudImportedPlugin;
  warnings: string[];
};

export type iPolloWalkCloudPluginsResult = {
  marketplaces: Record<string, CloudImportedMarketplace>;
  plugins: Record<string, CloudImportedPlugin>;
};

export type iPolloWalkClaudePluginComponent = {
  type: "mcp" | "skill" | "command" | "agent";
  name: string;
  description: string | null;
};

export type iPolloWalkClaudePluginPreview = {
  pluginId: string;
  name: string;
  description: string | null;
  version: string | null;
  source: { owner: string; repo: string; ref: string; dir: string | null };
  components: iPolloWalkClaudePluginComponent[];
  warnings: string[];
};
...
export type iPolloWalkCommandItem = {
  name: string;
  description?: string;
  template: string;
  agent?: string;
...
export type iPolloWalkMcpEngineSync = {
  status: "ok" | "failed";
  at: number;
  failures: Array<{ name: string; status?: number; message?: string }>;
};

export type iPolloWalkWorkspaceExport = {
  workspaceId: string;
  exportedAt: number;
...
export type iPolloWalkWorkspaceImportChange = {
  kind: "opencode" | "ipollowalk" | "skill" | "command" | "file";
  action: "create" | "update" | "replace" | "delete" | "unchanged";
  label: string;
  path: string;
};

export type iPolloWalkWorkspaceImportPreview = {
  fingerprint: string;
  summary: {
    total: number;
    create: number;
  ...
  changes: iPolloWalkWorkspaceImportChange[];
};

export type iPolloWalkWorkspaceExportSensitiveMode = "auto" | "include" | "exclude";

export type iPolloWalkWorkspaceExportWarning = {
  id: string;
  label: string;
  detail: string;
};

export type iPolloWalkBlueprintSessionsMaterializeResult = {
  ok: boolean;
...
export type iPolloWalkArtifactItem = {
  id: string;
  name?: string;
  path?: string;
  size?: number;
...
export type iPolloWalkArtifactList = {
  items: iPolloWalkArtifactItem[];
};
...
export type iPolloWalkConnectState = {
  ok: true;
  schemaVersion: 1;
...
export type iPolloWalkExtensionActionCall = {
  extensionId: string;
  action: string;
...
export type iPolloWalkExtensionActionResult =
  | {
    ok: true;
    extensionId: string;
    action: string;
...
export type iPolloWalkResolvedArtifactTarget = {
  id: string;
  kind: "file" | "url";
  value: string;
  name: string;
...
export type iPolloWalkWorkspaceFileStat = {
  ok: boolean;
  path: string;
  exists: boolean;
  kind?: "file" | "dir" | "other";
...
export type iPolloWalkInboxItem = {
  id: string;
  name?: string;
  path?: string;
  size?: number;
...
export type iPolloWalkInboxList = {
  items: iPolloWalkInboxItem[];
};

export type iPolloWalkInboxUploadResult = {
  ok: boolean;
  path: string;
  bytes: number;
};

export type iPolloWalkUserEnvItem = {
  key: string;
  updatedAt: number;
  hasValue: boolean;
  value?: string;
...
export type iPolloWalkActor = {
  type: "remote" | "host";
  clientId?: string;
  tokenHash?: string;
};

export type iPolloWalkAuditEntry = {
  id: string;
  workspaceId: string;
  actor: iPolloWalkActor;
  action: string;
  target: string;
  summary: string;
  timestamp: number;
...
export type iPolloWalkReloadTrigger = {
...
export type iPolloWalkReloadEvent = {
  id: string;
  seq: number;
  workspaceId: string;
  reason: "plugins" | "skills" | "mcp" | "config" | "agents" | "commands";
  trigger?: iPolloWalkReloadTrigger;
  timestamp: number;
};

export type iPolloWalkSessionGroupDefinition = {
  id: string;
  label: string;
};

export type iPolloWalkSessionGroupState = {
  groups: iPolloWalkSessionGroupDefinition[];
  assignments: Record<string, string>;
};

export type iPolloWalkSessionGroupEvent = {
  id: string;
  seq: number;
  workspaceId: string;
...
export const DEFAULT_IPOLLOWALK_SERVER_PORT = 8787;

const STORAGE_URL_OVERRIDE = "ipollowalk.server.urlOverride";
const STORAGE_PORT_OVERRIDE = "ipollowalk.server.port";
const STORAGE_TOKEN = "ipollowalk.server.token";
const STORAGE_HOST_AUTH_KEY = "ipollowalk.server.hostToken";
const STORAGE_REMOTE_ACCESS = "ipollowalk.server.remoteAccessEnabled";

export function normalizeiPolloWalkServerUrl(input: string) {
...
export function isLoopbackiPolloWalkServerUrl(input: string) {
  const normalized = normalizeiPolloWalkServerUrl(input) ?? "";
...
export function parseiPolloWalkWorkspaceIdFromUrl(input: string) {
  const normalized = normalizeiPolloWalkServerUrl(input) ?? "";
...
export function buildiPolloWalkWorkspaceBaseUrl(hostUrl: string, workspaceId?: string | null) {
  const normalized = normalizeiPolloWalkServerUrl(hostUrl) ?? "";
  if (!normalized) return null;
...
const IPOLLOWALK_INVITE_PARAM_URL = "ow_url";
const IPOLLOWALK_INVITE_PARAM_TOKEN = "ow_token";
const IPOLLOWALK_INVITE_PARAM_STARTUP = "ow_startup";
const IPOLLOWALK_INVITE_PARAM_AUTO_CONNECT = "ow_auto_connect";

export type iPolloWalkConnectInvite = {
  url: string;
  token?: string;
  startup?: "server";
  autoConnect?: boolean;
...
export function readiPolloWalkConnectInviteFromSearch(input: string | URLSearchParams) {
  const search =
    typeof input === "string"
      ? new URLSearchParams(input.startsWith("?") ? input.slice(1) : input)
      : input;

  const rawUrl = search.get(IPOLLOWALK_INVITE_PARAM_URL)?.trim() ?? "";
  const url = normalizeiPolloWalkServerUrl(rawUrl);
  if (!url) return null;

  const token = search.get(IPOLLOWALK_INVITE_PARAM_TOKEN)?.trim() ?? "";
  const startupRaw = search.get(IPOLLOWALK_INVITE_PARAM_STARTUP)?.trim() ?? "";
  const startup = startupRaw === "server" ? "server" : undefined;
  const autoConnect = search.get(IPOLLOWALK_INVITE_PARAM_AUTO_CONNECT)?.trim() === "1";
  ...
  } satisfies iPolloWalkConnectInvite;
}

export function stripiPolloWalkConnectInviteFromUrl(input: string) {
  try {
    const url = new URL(input);
    url.searchParams.delete(IPOLLOWALK_INVITE_PARAM_URL);
    url.searchParams.delete(IPOLLOWALK_INVITE_PARAM_TOKEN);
    url.searchParams.delete(IPOLLOWALK_INVITE_PARAM_STARTUP);
    url.searchParams.delete(IPOLLOWALK_INVITE_PARAM_AUTO_CONNECT);
    return url.toString();
  } catch {
    return input;
  }
...
export function readiPolloWalkServerSettings(): iPolloWalkServerSettings {
  if (typeof window === "undefined") return {};
  try {
    const urlOverride = normalizeiPolloWalkServerUrl(
...
export function writeiPolloWalkServerSettings(next: iPolloWalkServerSettings): iPolloWalkServerSettings {
  if (typeof window === "undefined") return next;
  try {
    const urlOverride = normalizeiPolloWalkServerUrl(next.urlOverride ?? "");
    ...
    return readiPolloWalkServerSettings();
  } catch {
    return next;
  }
}

export function hydrateiPolloWalkServerSettingsFromEnv() {
  if (typeof window === "undefined") return;

  const envUrl = typeof import.meta.env?.VITE_IPOLLOWALK_URL === "string"
    ? import.meta.env.VITE_IPOLLOWALK_URL.trim()
    : "";
  const envPort = typeof import.meta.env?.VITE_IPOLLOWALK_PORT === "string"
    ? import.meta.env.VITE_IPOLLOWALK_PORT.trim()
    : "";
  const envToken = typeof import.meta.env?.VITE_IPOLLOWALK_TOKEN === "string"
    ? import.meta.env.VITE_IPOLLOWALK_TOKEN.trim()
    : "";
  const envHostToken = typeof import.meta.env?.VITE_IPOLLOWALK_HOST_TOKEN === "string"
    ? import.meta.env.VITE_IPOLLOWALK_HOST_TOKEN.trim()
    : "";

  if (!envUrl && !envPort && !envToken && !envHostToken) return;

  try {
    const current = readiPolloWalkServerSettings();
    const next: iPolloWalkServerSettings = { ...current };
    let changed = false;

    if (!current.urlOverride && envUrl) {
      next.urlOverride = normalizeiPolloWalkServerUrl(envUrl) ?? undefined;
      changed = true;
    }
...
export function cleariPolloWalkServerSettings() {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(STORAGE_URL_OVERRIDE);
    window.localStorage.removeItem(STORAGE_PORT_OVERRIDE);
...
export class iPolloWalkServerError extends Error {
  status: number;
  code: string;
  details?: unknown;
...
function buildHeaders(
  ...
  if (hostToken) {
    headers["X-iPolloWalk-Host-Token"] = hostToken;
  }
  if (extra) {
...
const IPOLLOWALK_STREAM_URL_RE = /\/events(\b|\?)|\/event-stream\b|\/stream\b/;

function isStreamUrl(url: string): boolean {
  return IPOLLOWALK_STREAM_URL_RE.test(url);
}
...
const DEFAULT_IPOLLOWALK_SERVER_TIMEOUT_MS = 10_000;
...
async function requestJson<T>(
  ...
  const response = await fetchWithTimeout(
    ...
    options.timeoutMs ?? DEFAULT_IPOLLOWALK_SERVER_TIMEOUT_MS,
  );
  ...
  if (!response.ok) {
    ...
    throw new iPolloWalkServerError(response.status, code, message, json?.details);
  }
...
export function createiPolloWalkServerClient(options: { baseUrl: string; token?: string; hostToken?: string }) {
  ...
  return {
    ...
    runtimeVersions: () =>
      requestJson<iPolloWalkRuntimeSnapshot>(baseUrl, "/runtime/versions", { token, hostToken, timeoutMs: timeouts.status }),
    status: () => requestJson<iPolloWalkServerDiagnostics>(baseUrl, "/status", { token, hostToken, timeoutMs: timeouts.status }),
    capabilities: () => requestJson<iPolloWalkServerCapabilities>(baseUrl, "/capabilities", { token, hostToken, timeoutMs: timeouts.capabilities }),
    ...
    setConnectState: (connectEnabled: boolean) => requestJson<iPolloWalkConnectState>(baseUrl, "/experimental/connect/state", { token, hostToken, method: "PUT", body: { connectEnabled }, timeoutMs: timeouts.config }),
    ...
    callExtensionAction: (payload: iPolloWalkExtensionActionCall) =>
      requestJson<iPolloWalkExtensionActionResult>(baseUrl, "/experimental/extensions/call", {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    listWorkspaces: () => requestJson<iPolloWalkWorkspaceList>(baseUrl, "/workspaces", { token, hostToken, timeoutMs: timeouts.listWorkspaces }),
    ...
    createRemoteWorkspace: (payload: {
      ...
      remoteType?: "ipollowalk" | "opencode";
    ...
    activateWorkspace: (workspaceId: string, options?: { persist?: boolean }) => {
      ...
      return requestJson<{ activeId: string; workspace: iPolloWalkWorkspaceInfo; persisted: boolean }>(
        baseUrl,
    ...
    deleteWorkspace: (workspaceId: string) =>
      requestJson<{ ok: boolean; deleted: boolean; persisted: boolean; activeId: string | null; items: iPolloWalkWorkspaceInfo[]; workspaces?: WorkspaceInfo[] }>(
        baseUrl,
    ...
    getSessionGroups: (workspaceId: string) =>
      requestJson<{ state: iPolloWalkSessionGroupState; updatedAt: number | null }>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/session-groups`,
        { token, hostToken, timeoutMs: timeouts.sessionRead },
      ),
    putSessionGroups: (workspaceId: string, state: iPolloWalkSessionGroupState) =>
      requestJson<{ state: iPolloWalkSessionGroupState; updatedAt: number }>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/session-groups`,
    ...
    listSessionGroupEvents: (workspaceId: string, options?: { since?: number }) => {
      ...
      return requestJson<{ items: iPolloWalkSessionGroupEvent[]; cursor?: number }>(
        baseUrl,
    ...
    getSessionMessages: (workspaceId: string, sessionId: string, options?: { limit?: number }) => {
      ...
      if (typeof options?.limit === "number") query.set("limit", String(options.limit));
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<{ items: iPolloWalkSessionMessage[] }>(
        baseUrl,
    ...
    getSessionSnapshot: (workspaceId: string, sessionId: string, options?: { limit?: number }) => {
      ...
      if (typeof options?.limit === "number") query.set("limit", String(options.limit));
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<{ item: iPolloWalkSessionSnapshot }>(
        baseUrl,
    ...
    exportWorkspace: (
      workspaceId: string,
      options?: { sensitiveMode?: iPolloWalkWorkspaceExportSensitiveMode },
    ) => {
      const query = new URLSearchParams();
      ...
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<iPolloWalkWorkspaceExport>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/export${suffix}`, {
        token,
        hostToken,
    ...
    importWorkspace: (workspaceId: string, payload: Record<string, unknown>) =>
      requestJson<{ ok: boolean; preview?: iPolloWalkWorkspaceImportPreview }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/import`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    previewWorkspaceImport: (workspaceId: string, payload: Record<string, unknown>) =>
      requestJson<iPolloWalkWorkspaceImportPreview>(
        baseUrl,
    ...
    materializeBlueprintSessions: (workspaceId: string) =>
      requestJson<iPolloWalkBlueprintSessionsMaterializeResult>(
        baseUrl,
    ...
    getConfig: (workspaceId: string) =>
      requestJson<{ opencode: Record<string, unknown>; ipollowalk: Record<string, unknown>; updatedAt?: number | null }>(
        baseUrl,
        `/workspace/${workspaceId}/config`,
        { token, hostToken, timeoutMs: timeouts.config },
      ),
    listAuthorizedFolders: (workspaceId: string) =>
      requestJson<iPolloWalkAuthorizedFoldersResponse>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/authorized-folders`,
        { token, hostToken, timeoutMs: timeouts.config },
      ),
    setAuthorizedFolders: (workspaceId: string, folders: string[]) =>
      requestJson<iPolloWalkAuthorizedFoldersUpdateResponse>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/authorized-folders`,
        {
          token,
    ...
    migrateRuntimeConfig: (workspaceId: string) =>
      requestJson<iPolloWalkRuntimeConfigMigrationResult>(
        baseUrl,
    ...
    getRuntimeConfigStatus: (workspaceId: string) =>
      requestJson<iPolloWalkRuntimeConfigStatus>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/runtime-config`,
        { token, hostToken, timeoutMs: timeouts.config },
      ),
    patchConfig: (workspaceId: string, payload: { opencode?: Record<string, unknown>; ipollowalk?: Record<string, unknown> }) =>
    ...
    getDesktopCloudSync: (workspaceId: string) =>
      requestJson<iPolloWalkDesktopCloudSyncState>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/desktop-cloud-sync`, {
        token,
        hostToken,
        timeoutMs: timeouts.config,
      }),
    syncDesktopCloud: (workspaceId: string, snapshot: DenResourceSnapshot) =>
      requestJson<iPolloWalkDesktopCloudSyncResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/desktop-cloud-sync`, {
        token,
        hostToken,
        method: "POST",
        body: { snapshot },
    ...
    listCloudPlugins: (workspaceId: string) =>
      requestJson<iPolloWalkCloudPluginsResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/cloud-plugins`, {
        token,
        hostToken,
        timeoutMs: timeouts.config,
      }),
    installCloudPlugin: (workspaceId: string, payload: { marketplaceId: string | null; marketplace?: DenOrgMarketplace | null; resolved: DenOrgPluginResolved }) =>
      requestJson<iPolloWalkCloudPluginInstallResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/cloud-plugins`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    removeCloudPlugin: (workspaceId: string, pluginId: string) =>
      requestJson<iPolloWalkCloudPluginInstallResult>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/cloud-plugins/${encodeURIComponent(pluginId)}`, {
        token,
        hostToken,
        method: "DELETE",
        timeoutMs: timeouts.config,
    ...
    previewClaudePlugin: (workspaceId: string, payload: { url: string; ref?: string }) =>
      requestJson<{ preview: iPolloWalkClaudePluginPreview }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/claude-plugins`, {
        token,
        hostToken,
        method: "POST",
    ...
    installClaudePlugin: (workspaceId: string, payload: { url: string; ref?: string }) =>
      requestJson<iPolloWalkCloudPluginInstallResult & { preview: iPolloWalkClaudePluginPreview }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/claude-plugins`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    listReloadEvents: (workspaceId: string, options?: { since?: number }) => {
      const query = typeof options?.since === "number" ? `?since=${options.since}` : "";
      return requestJson<{ items: iPolloWalkReloadEvent[]; cursor?: number }>(
        baseUrl,
    ...
    listPlugins: (workspaceId: string, options?: { includeGlobal?: boolean }) => {
      ...
      return requestJson<{ items: iPolloWalkPluginItem[]; loadOrder: string[] }>(
        baseUrl,
    ...
    addPlugin: (workspaceId: string, spec: string) =>
      requestJson<{ items: iPolloWalkPluginItem[]; loadOrder: string[] }>(
        baseUrl,
    ...
    listSkills: (workspaceId: string, options?: { includeGlobal?: boolean }) => {
      const query = options?.includeGlobal ? "?includeGlobal=true" : "";
      return requestJson<{ items: iPolloWalkSkillItem[] }>(
        baseUrl,
        `/workspace/${workspaceId}/skills${query}`,
        { token, hostToken },
      );
    },
    listHubSkills: (options?: { repo?: iPolloWalkHubRepo }) => {
      ...
      return requestJson<{ items: iPolloWalkHubSkillItem[] }>(baseUrl, `/hub/skills${query}`, {
        token,
        hostToken,
      });
    },
    ...
    getSkill: (workspaceId: string, name: string, options?: { includeGlobal?: boolean }) => {
      const query = options?.includeGlobal ? "?includeGlobal=true" : "";
      return requestJson<iPolloWalkSkillContent>(
        baseUrl,
    ...
    upsertSkill: (workspaceId: string, payload: { name: string; content: string; description?: string }) =>
      requestJson<iPolloWalkSkillItem>(baseUrl, `/workspace/${workspaceId}/skills`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    listMcp: (workspaceId: string) =>
      requestJson<{ items: iPolloWalkMcpItem[]; engineSync?: iPolloWalkMcpEngineSync | null }>(
        baseUrl,
        `/workspace/${workspaceId}/mcp`,
        { token, hostToken },
      ),
    addMcp: (workspaceId: string, payload: { name: string; config: Record<string, unknown> }) =>
      requestJson<{ items: iPolloWalkMcpItem[] }>(baseUrl, `/workspace/${workspaceId}/mcp`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    removeMcp: (workspaceId: string, name: string) =>
      requestJson<{ items: iPolloWalkMcpItem[] }>(baseUrl, `/workspace/${workspaceId}/mcp/${encodeURIComponent(name)}`, {
        token,
        hostToken,
        method: "DELETE",
      }),
    setMcpEnabled: (workspaceId: string, name: string, enabled: boolean) =>
      requestJson<{ items: iPolloWalkMcpItem[] }>(
        baseUrl,
    ...
    upsertCommand: (
      ...
      requestJson<{ items: iPolloWalkCommandItem[] }>(baseUrl, `/workspace/${workspaceId}/commands`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    uploadInbox: async (workspaceId: string, file: File, options?: { path?: string }) => {
      ...
      if (!result.ok) {
        ...
        throw new iPolloWalkServerError(
          result.status,
          "request_failed",
      ...
      if (body) {
        try {
          const parsed = JSON.parse(body) as Partial<iPolloWalkInboxUploadResult>;
          if (typeof parsed.path === "string" && parsed.path.trim()) {
            return {
              ok: parsed.ok ?? true,
              path: parsed.path.trim(),
              bytes: typeof parsed.bytes === "number" ? parsed.bytes : file.size,
            } satisfies iPolloWalkInboxUploadResult;
          }
        } catch {
    ...
    listInbox: (workspaceId: string) =>
      requestJson<iPolloWalkInboxList>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/inbox`, {
        token,
        hostToken,
      }),
    ...
    readWorkspaceFile: (workspaceId: string, path: string) =>
      requestJson<iPolloWalkWorkspaceFileContent>(
        baseUrl,
    ...
    statWorkspaceFile: (workspaceId: string, path: string) =>
      requestJson<iPolloWalkWorkspaceFileStat>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/files/stat?path=${encodeURIComponent(path)}`,
        { token, hostToken },
      ),

    listWorkspaceFiles: async (workspaceId: string): Promise<iPolloWalkWorkspaceCatalogEntry[]> => {
      ...
      try {
        const items: iPolloWalkWorkspaceCatalogEntry[] = [];
        let after: string | undefined;
        do {
          const query = new URLSearchParams({ includeDirs: "false", limit: "1000" });
          if (after) query.set("after", after);
          const page = await requestJson<{
            items: iPolloWalkWorkspaceCatalogEntry[];
            truncated: boolean;
            nextAfter?: string;
          }>(
            baseUrl,
    ...
    writeWorkspaceFile: (
      ...
      requestJson<iPolloWalkWorkspaceFileWriteResult>(
        baseUrl,
    ...
    ): Promise<iPolloWalkWorkspaceFileDeleteResult[]> => {
    ...
    listArtifacts: (workspaceId: string) =>
      requestJson<iPolloWalkArtifactList>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/artifacts`, {
        token,
        hostToken,
      }),
    ...
    resolveArtifacts: (
      ...
      requestJson<{ items: iPolloWalkResolvedArtifactTarget[] }>(
        baseUrl,
    ...
    listUserEnv: () =>
      requestJson<{ items: iPolloWalkUserEnvItem[] }>(
        baseUrl,
    ...
    getUserEnv: (key: string) =>
      requestJson<{ item: iPolloWalkUserEnvItem & { value: string } }>(
        baseUrl,
...
export type iPolloWalkServerClient = ReturnType<typeof createiPolloWalkServerClient>;
