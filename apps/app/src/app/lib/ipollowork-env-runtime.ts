const PENDING_CHANGES_KEY = "ipollowork.settings.environment.pendingChanges";
...
export function buildiPolloWorkEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "ipollowork"}::${runtime || "runtime"}`;
}

export function readiPolloWorkEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeiPolloWorkEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
