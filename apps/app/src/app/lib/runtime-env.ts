...
// low-level clients (opencode, ipollowork-server, den) can use it without
// dragging in the utils barrel (which pulls i18n and app constants).
export function isElectronRuntime() {
  return typeof window !== "undefined" && (window as Window).__IPOLLOWORK_ELECTRON__ != null;
}
...
// True when the desktop was launched with IPOLLOWORK_DESKTOP_DISABLE_WORKSPACE_RECOVERY=1.
...
export function isDesktopWorkspaceRecoveryDisabled() {
  return (
    ...
    (window as Window).__IPOLLOWORK_ELECTRON__?.meta?.disableWorkspaceRecovery === true
  );
}
