...
import { normalizeiPolloWorkServerUrl } from "./ipollowork-server";
...
function isSupportedDeepLinkProtocol(protocol: string): boolean {
  ...
  return normalized === "ipollowork:" || normalized === "ipollowork-dev:" || normalized === "https:" || normalized === "http:";
}
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("ipolloworkHostUrl") ?? url.searchParams.get("ipolloworkUrl") ?? "";
  const tokenRaw = url.searchParams.get("ipolloworkToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeiPolloWorkServerUrl(hostUrlRaw);
  ...
  return {
    ipolloworkHostUrl: normalizedHostUrl,
    ipolloworkToken: token,
    directory: null,
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:ipollowork-dev|ipollowork|https?):\/\/[^\s"'<>]+/i);
