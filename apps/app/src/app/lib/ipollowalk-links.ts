...
import { normalizeiPolloWalkServerUrl } from "./ipollowalk-server";
...
function isSupportedDeepLinkProtocol(protocol: string): boolean {
  ...
  return normalized === "ipollowalk:" || normalized === "ipollowalk-dev:" || normalized === "https:" || normalized === "http:";
}
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("ipollowalkHostUrl") ?? url.searchParams.get("ipollowalkUrl") ?? "";
  const tokenRaw = url.searchParams.get("ipollowalkToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeiPolloWalkServerUrl(hostUrlRaw);
  ...
  return {
    ipollowalkHostUrl: normalizedHostUrl,
    ipollowalkToken: token,
    directory: null,
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:ipollowalk-dev|ipollowalk|https?):\/\/[^\s"'<>]+/i);
