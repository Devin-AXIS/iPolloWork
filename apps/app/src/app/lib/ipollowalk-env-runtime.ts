const PENDING_CHANGES_KEY = "ipollowalk.settings.environment.pendingChanges";
...
export function buildiPolloWalkEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "ipollowalk"}::${runtime || "runtime"}`;
}

export function readiPolloWalkEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeiPolloWalkEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
