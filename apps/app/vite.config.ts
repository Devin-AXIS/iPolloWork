...
addHost(process.env.IPOLLOWALK_PUBLIC_HOST ?? null);
...
const buildAppVersion =
  process.env.VITE_IPOLLOWALK_APP_VERSION?.trim() ||
...
const isElectronPackagedBuild = process.env.IPOLLOWALK_ELECTRON_BUILD === "1";
...
export default defineConfig({
  ...
  define: {
    ...
    "import.meta.env.VITE_IPOLLOWALK_APP_VERSION": JSON.stringify(buildAppVersion),
  },
  plugins: [
    {
      name: "ipollowalk-dev-server-id",
      configureServer(server) {
        server.middlewares.use("/__ipollowalk_dev_server_id", (_req, res) => {
