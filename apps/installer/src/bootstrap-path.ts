 ...
 * bootstrap CLI (packages/ipollowork-bootstrap/bin/ipollowork.mjs): XDG_CONFIG_HOME
...
export function desktopBootstrapPath(
  ...
  const override = env.IPOLLOWORK_DESKTOP_BOOTSTRAP_PATH?.trim()
...
export function legacyDesktopBootstrapPath(
  ...
  return path.join(home, ".config", "ipollowork", "desktop-bootstrap.json")
}
