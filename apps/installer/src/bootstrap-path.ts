 ...
 * bootstrap CLI (packages/ipollowalk-bootstrap/bin/ipollowalk.mjs): XDG_CONFIG_HOME
...
export function desktopBootstrapPath(
  ...
  const override = env.IPOLLOWALK_DESKTOP_BOOTSTRAP_PATH?.trim()
...
export function legacyDesktopBootstrapPath(
  ...
  return path.join(home, ".config", "ipollowalk", "desktop-bootstrap.json")
}
