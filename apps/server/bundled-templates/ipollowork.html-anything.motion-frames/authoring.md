...
Read `core-v1-index.md`, then `core-v1-video/catalog.md`, `core-v1-video/layout.md` and `core-v1-video/shared-contract.md` beside the brief. The app-owned library supplies independent scene bodies, capacity guidance and static previews. Copy only the fitting fragment and scoped layout styles, and bind them to this template's existing semantic tokens. The preview palette and preview host are not part of the layout. Prefer the best-fitting local or shared layout; do not use a library layout just to demonstrate variety.
