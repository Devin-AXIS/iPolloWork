...
test("renames independent output, updates references and rejects collisions", async () => {
  const root = await mkdtemp(join(tmpdir(), "ipollowork-rename-test-"));
  ...
  try {
    ...
    await recordSessionArtifact(config, workspace, "session", "old.png");
    ...
    expect((await listSessionArtifacts(config, "w", "session")).items).toEqual([expect.objectContaining({ path: "new.png", previousPaths: ["old.png"] })]);
