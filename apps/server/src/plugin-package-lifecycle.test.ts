...
describe("plugin package lifecycle", () => {
  ...
  test("installs the Xiaohongshu plugin with isolated browser login configuration", async () => {
    ...
    try {
      ...
      expect((await catalog.json()).items).toContainEqual(expect.objectContaining({ pluginId: "xiaohongshu-ops", version: "0.3.13" }));
    ...
    try {
      ...
      expect(await catalog.json()).toMatchObject({
        items: [
          ...
          { pluginId: "xiaohongshu-ops", version: "0.3.13", installedVersion: null, updateAvailable: false },
        ],
      });
