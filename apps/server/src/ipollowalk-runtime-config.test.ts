...
import {
  keepiPolloWalkRuntimeConfigFileFresh,
  ipollowalkRuntimeConfigFilePath,
  writeiPolloWalkRuntimeConfigFile,
} from "./ipollowalk-runtime-config.js";
...
async function setup() {
  const root = await mkdtemp(join(tmpdir(), "ipollowalk-runtime-config-file-"));
  roots.push(root);
  previousDb = process.env.IPOLLOWALK_RUNTIME_DB;
...
async function readConfigFile(config: ServerConfig): Promise<Record<string, unknown>> {
  const raw = await readFile(ipollowalkRuntimeConfigFilePath(config), "utf8");
  return JSON.parse(raw) as Record<string, unknown>;
}

describe("ipollowalk runtime config file", () => {
  test("writes runtime-DB MCPs and ipollowalk defaults into the file", async () => {
    ...
    const path = await writeiPolloWalkRuntimeConfigFile(config, "ws_1");
    expect(path).toBe(ipollowalkRuntimeConfigFilePath(config));
    ...
    expect(parsed.default_agent).toBe("ipollowalk");
    expect(Array.isArray(parsed.plugin)).toBe(true);
  });

  test("ipollowalk prompt has a static search-first Memory Bank section, distinct from ## Memory", async () => {
    const { config } = await setup();
    await writeiPolloWalkRuntimeConfigFile(config, "ws_1");

    const parsed = await readConfigFile(config);
    const agent = parsed.agent as Record<string, { prompt?: string }>;
    const prompt = agent.ipollowalk?.prompt ?? "";
  ...
  test("keepiPolloWalkRuntimeConfigFileFresh rewrites the file on runtime-DB writes", async () => {
    const { config } = await setup();
    await writeiPolloWalkRuntimeConfigFile(config, "ws_1");
    cleanups.push(keepiPolloWalkRuntimeConfigFileFresh(config, "ws_1"));
