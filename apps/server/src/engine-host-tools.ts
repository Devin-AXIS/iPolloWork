...
- For editable Video Studio generation, component-map selection is not component reuse. After selecting registry IDs, call ipollowork_extension_call with extensionId=media, action=video_component_install, args={sourcePath:"video/<exact-project-id>/index.html",componentIds:[...]}; integrate the returned host-timed data-composition-src snippets, authored targets, real variables, data-ipw-timing-owner=host, data-ipw-timing-source and continuous data-ipw-beats. The returned motionContract declares native duration and targets only; measure actual establish, develop, and land windows from the saved render. Every beat must bind CSS targets to an executable component, preset, custom, or explicit hold reference and record a truthful scene-relative motion start/end window. An intentional hold may last up to four seconds; split longer static intervals at a real narration or content boundary. Apply presets to text or elements through list_motion_presets plus mutate_motion with explicit start/end and preserve data-ipw-animation-reference. Every scene after the first declares an incoming transition, duration and narrative intent; scene windows remain contiguous and the transition runs inside the incoming scene. For music-led audio-reactive work, call media/video_audio_analyze once after the real local music file is mounted, then copy a bounded set of returned measured cues into each affected scene's data-ipw-audio-cues and bind every cue to an active beat motion window. Never infer cue timestamps from duration or a waveform image. Save once and return; the client runs the aggregate component and delivery validator and may request one bounded repair continuation. A selected component that was redrawn locally does not count as reused.
- Missing or disabled narration authorization never blocks editable video generation. Continue with a complete silent composition, use estimated-reading or the actual visual/music/media cue as the scene timing source, and disclose the silent result without opening settings or leaving a pending-model placeholder. If narration becomes available later, preserve scene meaning and retime beats from the measured audio.
