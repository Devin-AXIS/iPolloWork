 ...
 *
 * This is the single source of truth for the ipollowalk agent definition,
...
import {
  ipollowalkExtensionsPreviewPluginPath,
  ipollowalkCapabilitiesKnowledgePluginPath,
  ipollowalkAnthropicAdaptiveThinkingPluginPath,
  ipollowalkAnthropicToolSchemaPluginPath,
} from "./ipollowalk-extensions-plugin-path.js";
...
const IPOLLOWALK_AGENT_PROMPT = `You are iPolloWalk.

When the user refers to "you", they mean the iPolloWalk app and the current workspace.
...
## iPolloWalk Artifacts

iPolloWalk can preview, edit, and download standard artifacts when you create or update them in the workspace.
...
export async function buildiPolloWalkRuntimeConfigObject(
  config?: ServerConfig,
  workspaceId?: string,
  ...
  return {
    ...runtimeConfig,
    default_agent: runtimeConfig.default_agent ?? "ipollowalk",
    agent: {
      ipollowalk: {
        description: "iPolloWalk default agent",
        mode: "primary",
        temperature: 0.2,
        prompt: IPOLLOWALK_AGENT_PROMPT,
      },
    },
    plugin: [
...
export async function buildiPolloWalkRuntimeConfig(config?: ServerConfig, workspaceId?: string): Promise<string> {
  return JSON.stringify(await buildiPolloWalkRuntimeConfigObject(config, workspaceId));
}

export function ipollowalkRuntimeConfigFilePath(config: ServerConfig): string {
 ...
 * mid-dispose.
 */
export async function writeiPolloWalkRuntimeConfigFile(config: ServerConfig, workspaceId: string): Promise<string> {
  const path = ipollowalkRuntimeConfigFilePath(config);
  const job = async () => {
    const content = await buildiPolloWalkRuntimeConfig(config, workspaceId);
 ...
 */
export function keepiPolloWalkRuntimeConfigFileFresh(config: ServerConfig, workspaceId: string): () => void {
  return onRuntimeOpencodeConfigWrite((writeConfig, writtenWorkspaceId) => {
    if (writtenWorkspaceId !== workspaceId) return;
    void writeiPolloWalkRuntimeConfigFile(writeConfig, workspaceId).catch(() => undefined);
  });
}
