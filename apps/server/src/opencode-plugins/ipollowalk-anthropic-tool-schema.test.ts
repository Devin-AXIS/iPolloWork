...
import { iPolloWalkAnthropicToolSchema } from "./ipollowalk-anthropic-tool-schema.js";
...
beforeAll(async () => {
  ...
  await iPolloWalkAnthropicToolSchema();
...
describe("iPolloWalkAnthropicToolSchema fetch patch", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./ipollowalk-anthropic-tool-schema.js");
    expect(Object.keys(mod)).toEqual(["iPolloWalkAnthropicToolSchema"]);
  });
});
