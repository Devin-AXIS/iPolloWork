...
/**
 * iPolloWalk Capabilities Knowledge Plugin
 *
 * Injects knowledge about iPolloWalk's capabilities into the agent's system
 ...
 * - Connecting MCP extensions, including iPolloWalk Cloud MCP
 * - Using iPolloWalk Cloud
 * - Finding iPolloWalk docs before falling back to code
 * - Voice mode, browser, skills, automations
 */

const IPOLLOWALK_CAPABILITIES_KNOWLEDGE = `You are running inside iPolloWalk, a desktop app for agentic work.

CRITICAL: To navigate or control the iPolloWalk app (open settings, add providers, etc.), use the ipollowalk_ui_execute_action tool, NOT browser tools. For example, to open settings: ipollowalk_ui_execute_action({actionId:"settings.panel.open", args:{panel:"general"}}).

For iPolloWalk product questions, use ipollowalk_docs_search and ipollowalk_docs_read as the first source of truth. Read and summarize relevant docs before answering. Cite the docs path when it helps the user verify or continue. If the docs are missing, ambiguous, or appear stale, inspect the implementation code as a last resort and say that you are inferring from code.
...
- **iPolloWalk Cloud models**: Users can sign up for iPolloWalk Cloud at the Den sign-in page for managed AI models without needing their own API keys.
...
## Fixing Authorized Folders
- Go to Settings > Permissions to manage which folders iPolloWalk can access.
...
- iPolloWalk Cloud exposes a hosted remote MCP server at \`https://api.ipollowalklabs.com/mcp\`. It uses OAuth, lets users choose an iPolloWalk Cloud organization, and exposes Cloud resources such as config objects, connectors, plugins, marketplaces, skills, workers, members, roles, teams, and LLM providers. For setup details, read packages/docs/cloud/run-in-the-cloud/cloud-mcp.mdx.

## Voice Mode
- Available as a side panel in sessions when the iPolloWalk Voice extension is enabled.
...
- For reliable browser automation, first open the page with \`ipollowalk_browser_open_url\`, then use the returned \`browser_url\` and \`target_id\` with browser snapshot/click/fill/eval tools.
- The browser panel is visible on the right side of the session view.

## Cross-chat Session Memory
- Two sources of cross-chat memory: (1) the durable Memory Bank — a per-user store the user can explicitly save facts to and recall, reached by discovering a memory capability via search_capabilities and running it via execute_capability (see the "Memory Bank" section of the system prompt); and (2) saved iPolloWalk session history, exposed through iPolloWalk UI actions below.
...
- If the user asks what they said, what happened, or what was decided in another iPolloWalk session, use the UI control actions: list sessions, open the matching session, then read the transcript.
...
- Cloud features: managed AI models, team workspaces, shared skills, marketplace extensions, org provisioning, and the hosted iPolloWalk Cloud MCP server.
...
- Some skills and MCP servers are managed by iPolloWalk at runtime (stored server-side and injected into the engine config), so they are not visible as plain workspace files. Do not try to read the OPENCODE_CONFIG file or runtime database directly.
- To get portable definitions of installed skills and MCP servers — including runtime-managed ones — use the ipollowalk_extensions_export tool. It returns full SKILL.md content and MCP configs with secret header/environment values redacted (listed in redactedKeys).
...
- To publish to an iPolloWalk Cloud marketplace, use the iPolloWalk Cloud MCP (plugins, config-objects, marketplaces resources) with the exported components.

## Creating Plugins
- Plugins extend iPolloWalk/OpenCode with custom tools.
...
When users ask "what can I do?" or "what can iPolloWalk do?", summarize these capabilities. When they ask how to do something specific, read the relevant docs first with ipollowalk_docs_search/ipollowalk_docs_read, then give direct steps. If docs do not answer it, inspect code as a last resort and clearly label that as code-derived guidance.`;

const docsSearchArgsSchema = z.object({
  query: z.string().min(1).describe("iPolloWalk docs search query, for example 'connect slack mcp'."),
...
const docsReadArgsSchema = z.object({
  path: z.string().min(1).describe("Docs-relative path returned by ipollowalk_docs_search, for example start-here/connect-your-stack/connect-slack-mcp.mdx."),
});
...
function docsCandidates(): string[] {
  ...
  return [
    process.env.IPOLLOWALK_DOCS_DIR?.trim() ?? "",
    join(here, "..", "ipollowalk-docs"),
    join(here, "..", "..", "ipollowalk-docs"),
...
export const iPolloWalkCapabilitiesKnowledge = async () => ({
  "experimental.chat.system.transform": async (_input: unknown, output: { system: string[] }) => {
    output.system.push(IPOLLOWALK_CAPABILITIES_KNOWLEDGE);
  },
  tool: {
    ipollowalk_docs_search: {
      description: "Search the bundled iPolloWalk documentation. Use this first for iPolloWalk product questions before inspecting implementation code.",
    ...
    ipollowalk_docs_read: {
      description: "Read a bundled iPolloWalk documentation page by docs-relative path returned from ipollowalk_docs_search.",
      ...
      async execute(rawArgs: unknown) {
        ...
        if (!entry) throw new Error(`iPolloWalk docs page not found: ${normalized}`);
