...
import { iPolloWalkExtensionsPreview } from "./ipollowalk-extensions-preview.js";

const originalServerUrl = process.env.IPOLLOWALK_SERVER_URL;
const originalServerToken = process.env.IPOLLOWALK_SERVER_TOKEN;
const originalUiControlTools = process.env.IPOLLOWALK_UI_CONTROL_TOOLS;
...
afterEach(() => {
  ...
  if (originalUiControlTools === undefined) delete process.env.IPOLLOWALK_UI_CONTROL_TOOLS;
  else process.env.IPOLLOWALK_UI_CONTROL_TOOLS = originalUiControlTools;
});

async function transformedSystem(plugin: Awaited<ReturnType<typeof iPolloWalkExtensionsPreview>>): Promise<string> {
  const output: { system: string[] } = { system: [] };
  await plugin["experimental.chat.system.transform"]({}, output);
  return output.system.join("\n");
}

function startFakeiPolloWalkServer() {
...
describe("iPolloWalkExtensionsPreview session tools", () => {
  test("searches past chat transcript text and prefers the user's matching message", async () => {
    const fake = startFakeiPolloWalkServer();
    const plugin = await iPolloWalkExtensionsPreview();

    const output = await plugin.tool.ipollowalk_session_search.execute({
      query: "raven launch",
      limit: 5,
      scanLimit: 10,
    });
  ...
  test("reads a transcript by session id without opening the UI", async () => {
    startFakeiPolloWalkServer();
    const plugin = await iPolloWalkExtensionsPreview();

    const output = await plugin.tool.ipollowalk_session_read.execute({
...
describe("iPolloWalkExtensionsPreview UI control tools", () => {
  test("omits UI-control tools and steering by default", async () => {
    delete process.env.IPOLLOWALK_UI_CONTROL_TOOLS;
    const plugin = await iPolloWalkExtensionsPreview();
    const tools = Object.keys(plugin.tool);

    expect(tools).not.toContain("ipollowalk_ui_snapshot");
    expect(tools).not.toContain("ipollowalk_ui_list_actions");
    expect(tools).not.toContain("ipollowalk_ui_execute_action");
    expect(tools).toContain("ipollowalk_session_search");
    expect(tools).toContain("ipollowalk_extension_list_actions");

    const system = await transformedSystem(plugin);
    expect(system).not.toContain("ipollowalk_ui_");
    expect(system).toContain("ipollowalk_session_search");
  });

  test("registers UI-control tools and steering when opted in", async () => {
    process.env.IPOLLOWALK_UI_CONTROL_TOOLS = "1";
    const plugin = await iPolloWalkExtensionsPreview();
    const tools = Object.keys(plugin.tool);

    expect(tools).toContain("ipollowalk_ui_snapshot");
    expect(tools).toContain("ipollowalk_ui_list_actions");
    expect(tools).toContain("ipollowalk_ui_execute_action");

    const system = await transformedSystem(plugin);
    expect(system).toContain("ipollowalk_ui_execute_action");
  });
});
