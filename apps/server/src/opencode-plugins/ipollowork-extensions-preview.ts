...
const callArgsSchema = z.object({
  ...
  action: z.string().describe("Action id from ipollowork_extension_list_actions."),
...
const browserSetProxyArgsSchema = z.object({
  proxy: z.string().describe("Proxy URL like http://user:pass@host:8080 or socks5://host:1080. Prefer env:NAME (resolves the IPOLLOWORK_BROWSER_PROXY_NAME environment variable on the user's machine) so credentials never enter the conversation."),
});

const sessionSearchArgsSchema = z.object({
  query: z.string().trim().min(1).describe("Text to search for across iPolloWork session titles and message transcripts."),
  workspaceId: z.string().trim().optional().describe("Optional iPolloWork workspace id/name to limit the search."),
...
const sessionReadArgsSchema = z.object({
  sessionId: z.string().trim().min(1).describe("iPolloWork/OpenCode session ID returned by ipollowork_session_search."),
  workspaceId: z.string().trim().optional().describe("Optional iPolloWork workspace id/name. Omit to resolve the session across all workspaces."),
...
export async function resolveiPolloWorkExtensionDiscoveryInstruction(fetcher: iPolloWorkFetch = fetch, now: Clock = Date.now): Promise<string> {
...
const IPOLLOWORK_UI_CONTROL_INSTRUCTION =
  `IMPORTANT: You are running inside the iPolloWork desktop app. When the user asks you to open settings, navigate the app, add providers, or control the iPolloWork UI in any way, ALWAYS use the ipollowork_ui_* tools — NOT the browser_* tools. The browser tools are for external websites only. The ipollowork_ui_* tools control the app directly and are instant (one tool call).

To open settings: ipollowork_ui_execute_action with actionId "settings.panel.open" and args {panel:"general"} (or "ai", "extensions", "permissions", "skills", "appearance", etc.)
To add a provider: ipollowork_ui_execute_action with actionId "settings.provider.add" and optional args {providerId:"anthropic"}
To see what the user sees: ipollowork_ui_snapshot
To list all available actions: ipollowork_ui_list_actions
To ask what iPolloWork can do: ipollowork_ui_execute_action with actionId "help.capabilities"`;

const IPOLLOWORK_SESSION_MEMORY_INSTRUCTION =
...
// Settings -> Extensions. Set IPOLLOWORK_UI_CONTROL_TOOLS=1 to re-enable the
...
async function serverGet(path: string): Promise<unknown> {
  ...
  if (!response.ok) throw new Error(errorMessage(payload, "iPolloWork server request failed"));
  return payload;
}
...
function workspaceLabel(workspace: iPolloWorkWorkspace): string {
...
function titleSearchResult(workspace: iPolloWorkWorkspace, session: SessionInfo, queryLower: string): SessionSearchResult | null {
...
function messageSearchResult(workspace: iPolloWorkWorkspace, session: SessionInfo, messages: SessionMessage[], queryLower: string): SessionSearchResult | null {
...
async function listiPolloWorkWorkspaces(): Promise<iPolloWorkWorkspace[]> {
  return workspaceListEnvelopeSchema.parse(await serverGet("/workspaces")).items;
}

function filterWorkspaces(workspaces: iPolloWorkWorkspace[], workspaceId?: string): iPolloWorkWorkspace[] {
...
async function listWorkspaceSessions(workspace: iPolloWorkWorkspace, limit: number): Promise<SessionInfo[]> {
...
async function readWorkspaceSession(workspace: iPolloWorkWorkspace, sessionId: string): Promise<SessionInfo> {
...
async function readSessionMessages(workspace: iPolloWorkWorkspace, sessionId: string, limit: number): Promise<SessionMessage[]> {
...
async function searchiPolloWorkSessions(rawArgs: unknown): Promise<object> {
  ...
  const workspaces = filterWorkspaces(await listiPolloWorkWorkspaces(), args.workspaceId);
  if (!workspaces.length) {
    return { ok: false, error: args.workspaceId ? `No workspace matched ${args.workspaceId}` : "No iPolloWork workspaces are available" };
  }

  const sessions: Array<{ workspace: iPolloWorkWorkspace; session: SessionInfo }> = [];
...
async function readiPolloWorkSession(rawArgs: unknown): Promise<object> {
  ...
  return { ok: false, error: `Session ${args.sessionId} was not found in matching iPolloWork workspaces` };
}
...
async function resolveContextWorkspace(workspaceId: string | undefined, context: OpenCodeContext): Promise<iPolloWorkWorkspace> {
  const workspaces = await listiPolloWorkWorkspaces();
  if (!workspaces.length) throw new Error("No iPolloWork workspaces are available");
  if (workspaceId) {
  ...
  throw new Error(`Multiple iPolloWork workspaces match; pass workspaceId. Available: ${workspaces.map((workspace) => workspaceLabel(workspace)).join(", ")}`);
}

async function exportiPolloWorkExtensions(rawArgs: unknown, context: OpenCodeContext): Promise<object> {
...
async function postJson(path: string, body: ExtensionActionPayload | Record<string, unknown>): Promise<unknown> {
  ...
  const payload = await parseResponse(response);
  if (!response.ok) {
    throw new Error(errorMessage(payload, "iPolloWork extension call failed"));
  }
  return payload;
}
...
export const iPolloWorkExtensionsPreview = async () => {
  ...
  tool: {
    ...
    ipollowork_ui_execute_action: {
      description: `Execute an iPolloWork UI action by its id. Use ipollowork_ui_list_actions first to see available actions. ${IPOLLOWORK_UI_CONTROL_INSTRUCTION}`,
