...
import { iPolloWorkAnthropicAdaptiveThinking } from "./ipollowork-anthropic-adaptive-thinking.js";

async function runHook(apiId: string, options: Record<string, unknown>) {
  const hooks = await iPolloWorkAnthropicAdaptiveThinking();
  const output: { options: Record<string, unknown> } = { options };
  await hooks["chat.params"]({ model: { id: apiId, api: { id: apiId } } }, output);
  return output.options;
}

describe("iPolloWorkAnthropicAdaptiveThinking chat.params", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./ipollowork-anthropic-adaptive-thinking.js");
    expect(Object.keys(mod)).toEqual(["iPolloWorkAnthropicAdaptiveThinking"]);
  });
});
