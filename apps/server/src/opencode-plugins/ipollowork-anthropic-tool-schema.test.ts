...
import { iPolloWorkAnthropicToolSchema } from "./ipollowork-anthropic-tool-schema.js";
...
beforeAll(async () => {
  ...
  await iPolloWorkAnthropicToolSchema();
...
describe("iPolloWorkAnthropicToolSchema fetch patch", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./ipollowork-anthropic-tool-schema.js");
    expect(Object.keys(mod)).toEqual(["iPolloWorkAnthropicToolSchema"]);
  });
});
