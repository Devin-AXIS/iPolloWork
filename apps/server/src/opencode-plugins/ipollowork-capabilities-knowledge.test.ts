...
import { iPolloWorkCapabilitiesKnowledge } from "./ipollowork-capabilities-knowledge.js";

describe("iPolloWork capabilities knowledge plugin", () => {
  test("retrieves Slack connection guidance from bundled docs", async () => {
    process.env.IPOLLOWORK_DOCS_DIR = resolve(import.meta.dir, "../../../../packages/docs");

    const plugin = await iPolloWorkCapabilitiesKnowledge();
    const search = await plugin.tool.ipollowork_docs_search.execute({ query: "how can i connect slack", limit: 3 });

    expect(search).toContain("start-here/connect-your-stack/connect-slack-mcp.mdx");
    expect(search).toContain("Connect Slack MCP");

    const read = await plugin.tool.ipollowork_docs_read.execute({
