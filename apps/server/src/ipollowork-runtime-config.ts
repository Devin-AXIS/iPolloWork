 ...
 *
 * This is the single source of truth for the ipollowork agent definition,
...
When the user refers to "you", they mean the iPolloWork app and the current workspace.
...
## iPolloWork Artifacts

iPolloWork can preview, edit, and download standard artifacts when you create or update them in the workspace.
...
export async function buildiPolloWorkRuntimeConfigObject(
  config?: ServerConfig,
  workspaceId?: string,
...
export async function buildiPolloWorkRuntimeConfig(config?: ServerConfig, workspaceId?: string): Promise<string> {
  return JSON.stringify(await buildiPolloWorkRuntimeConfigObject(config, workspaceId));
}

export function ipolloworkRuntimeConfigFilePath(config: ServerConfig): string {
 ...
 * mid-dispose.
 */
export async function writeiPolloWorkRuntimeConfigFile(config: ServerConfig, workspaceId: string): Promise<string> {
  const path = ipolloworkRuntimeConfigFilePath(config);
  const job = async () => {
    const content = await buildiPolloWorkRuntimeConfig(config, workspaceId);
