#!/usr/bin/env bash
#
# ipollowalk-debug.sh — one-stop observability + lifecycle control for the
# iPolloWalk dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan ipollowalk/opencode processes (ppid == launchd)
...
#   wait-healthy    block until ipollowalk-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Variant (IPOLLOWALK_DEV_VARIANT):
#   electron   (default) pnpm --filter @ipollowalk/desktop dev:electron
...
#   3. Tauri webview       (target/debug/iPolloWalk-Dev)  <-- never /Applications/
#   4. Electron main+helpers (node_modules/electron/...Electron.app)
#   5. Vite                (node node_modules/.../vite)
#   6. orchestrator + ipollowalk-server + opencode
...
#   - ~/Library/Application Support/com.differentai.ipollowalk.dev/** (tokens,
#     workspaces registry, prefs). Use `reset-webview` for WebKit state.
#   - /Applications/iPolloWalk.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${IPOLLOWALK_DEV_LOG_FILE:-$HOME/.ipollowalk/debug/ipollowalk-dev.log}"
PNPM_DEV_LOG="${IPOLLOWALK_PNPM_DEV_LOG:-/tmp/ipollowalk-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${IPOLLOWALK_PNPM_DEV_PID:-/tmp/ipollowalk-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${IPOLLOWALK_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${IPOLLOWALK_ELECTRON_REMOTE_DEBUG_PORT:-9823}"
...
DEV_VARIANT="${IPOLLOWALK_DEV_VARIANT:-electron}"
case "$DEV_VARIANT" in electron|tauri) ;; *)
  printf '[ipollowalk-debug] unknown IPOLLOWALK_DEV_VARIANT=%s (expected electron|tauri)\n' "$DEV_VARIANT" >&2
  exit 2
  ;;
esac
...
kill_by_pattern() {
  ...
  # webview (matched by its target/debug path, so prod iPolloWalk.app is safe).
  local pattern="$1"
  local pids
...
discover_ipollowalk_server_port() {
  ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/ipollowalk-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.differentai\.ipollowalk/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.IPOLLOWALK_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/target\/debug\/iPolloWalk-Dev|node_modules\/electron\/dist\/Electron\.app\/Contents\/MacOS\/Electron|apps\/desktop\/scripts\/electron-dev\.mjs|target\/debug\/ipollowalk-server|target\/debug\/ipollowalk-orchestrator|target\/debug\/opencode( |\/)|apps\/desktop\/src-tauri\/sidecars\/ipollowalk-server|apps\/desktop\/src-tauri\/sidecars\/ipollowalk-orchestrator|apps\/desktop\/src-tauri\/sidecars\/opencode( |\/)|vite|pnpm .*dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== ipollowalk-server ==="
  local port
  port=$(discover_ipollowalk_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev ipollowalk-server running)"
  else
  ...
  oc_port=$(ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/ipollowalk-orchestrator" \
    | grep -v grep \
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /ipollowalk-server|ipollowalk-orchestrator|opencode( |\/)/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with IPOLLOWALK_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /ipollowalk-server|ipollowalk-orchestrator|opencode( |\/)/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  echo "=== ipollowalk hang diagnosis ==="
  echo "time=$now"
  echo "repo=$REPO_ROOT"
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(IPOLLOWALK_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_ipollowalk_server_port)
  if [[ -n "$port" ]]; then
    echo "  ipollowalk-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no ipollowalk-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/ipollowalk-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set IPOLLOWALK_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  kill_by_pattern "target/debug/iPolloWalk-Dev"
  ...
  # 5. ipollowalk-server / orchestrator / opencode for the
  ...
  kill_by_pattern "target/debug/ipollowalk-server"
  kill_by_pattern "target/debug/ipollowalk-orchestrator"
  kill_by_pattern "target/debug/opencode"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/ipollowalk-server"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/ipollowalk-orchestrator"
...
start() {
  ...
  case "$DEV_VARIANT" in
    electron)
      ...
      env IPOLLOWALK_DEV_LOG_FILE="$DEV_LOG_FILE" \
        nohup pnpm --filter @ipollowalk/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
      pid=$!
      ;;
    tauri)
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "ipollowalk-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "ipollowalk-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the ipollowalk-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.differentai.ipollowalk.dev"
