param(
    [string]$Hostname = "poc.ipollowalk.test",
...
function Stop-ReproJobs {
    try {
        $jobs = Get-Job -Name "iPolloWalkTlsRepro-*" -ErrorAction SilentlyContinue
        foreach ($job in $jobs) {
...
function Invoke-Cleanup {
    ...
    if (-not $Quiet) {
        Write-Step "Cleaning iPolloWalk TLS repro artifacts..."
    }
...
function Start-ReproListenerJob {
    ...
    $jobName = "iPolloWalkTlsRepro-{0}" -f $Port
    ...
    $job = Start-Job -Name $jobName -ArgumentList $Port, $Label -ScriptBlock {
        ...
        try {
            while ($true) {
                ...
                $context.Response.Headers.Add("X-iPolloWalk-TLS-Repro", $Label)
...
Write-Step "iPolloWalk TLS repro setup"
...
$rootSubject = "CN=iPolloWalk TLS Repro Root CA, O=$Marker"
$healthyIntermediateSubject = "CN=iPolloWalk TLS Repro Healthy Intermediate CA, O=$Marker"
$brokenIntermediateSubject = "CN=iPolloWalk TLS Repro Broken Intermediate CA, O=$Marker"
...
Write-Step ("   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\ipollowalk-doctor.ps1 -WebUrl {0} -ApiUrl {1} -ExpectedIssuerMatch `"iPolloWalk TLS Repro`"" -f $healthyUrl, $brokenUrl)
...
Write-Step "   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\setup-ipollowalk-tls-repro.ps1 -Cleanup"
