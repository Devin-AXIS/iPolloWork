param(
    [string]$Hostname = "poc.ipollowork.test",
...
function Stop-ReproJobs {
    try {
        $jobs = Get-Job -Name "iPolloWorkTlsRepro-*" -ErrorAction SilentlyContinue
        foreach ($job in $jobs) {
...
function Invoke-Cleanup {
    ...
    if (-not $Quiet) {
        Write-Step "Cleaning iPolloWork TLS repro artifacts..."
    }
...
function Start-ReproListenerJob {
    ...
    $jobName = "iPolloWorkTlsRepro-{0}" -f $Port
    ...
    $job = Start-Job -Name $jobName -ArgumentList $Port, $Label -ScriptBlock {
        ...
        try {
            while ($true) {
                ...
                $context.Response.Headers.Add("X-iPolloWork-TLS-Repro", $Label)
...
Write-Step "iPolloWork TLS repro setup"
...
$rootSubject = "CN=iPolloWork TLS Repro Root CA, O=$Marker"
$healthyIntermediateSubject = "CN=iPolloWork TLS Repro Healthy Intermediate CA, O=$Marker"
$brokenIntermediateSubject = "CN=iPolloWork TLS Repro Broken Intermediate CA, O=$Marker"
...
Write-Step ("   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\ipollowork-doctor.ps1 -WebUrl {0} -ApiUrl {1} -ExpectedIssuerMatch `"iPolloWork TLS Repro`"" -f $healthyUrl, $brokenUrl)
...
Write-Step "   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\setup-ipollowork-tls-repro.ps1 -Cleanup"
