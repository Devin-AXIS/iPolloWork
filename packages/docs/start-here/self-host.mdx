---
title: "Self-host"
description: "Run iPolloWork on your own server or infrastructure."
---

Most users just use the [desktop app](https://ipolloworklabs.com/download). Self-hosting is for teams that want iPolloWork to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

Want hosted workspaces, team marketplaces, or shared providers without running infra? Use [iPolloWork Cloud](/cloud/get-started) instead. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5) about [Enterprise](/cloud/enterprise).

## What you deploy

iPolloWork is split into normal services:

- **iPolloWork app**: the desktop or web client people use.
...
- **Inference service**: optional iPolloWork Models proxy and metering service.

If you only need one private remote workspace, deploy the worker runtime. If you want iPolloWork Cloud-style accounts, teams, and organization management, deploy Den web, Den controller, and optionally the inference service.

For Kubernetes installs, use the published Helm chart:

```bash
helm upgrade --install ipollowork-ee oci://ghcr.io/different-ai/charts/ipollowork-ee \
  --version <chart-version> \
  -f values.prod.yaml
```

The chart pulls the matching GHCR images for `ipollowork-den-api`, `ipollowork-den-web`, and optional `ipollowork-inference`.
...
For containerized services, start from the [Docker packaging](https://github.com/Devin-AXIS/iPolloWork/tree/dev/packaging/docker) in the repo.
...
- Hosted iPolloWork uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Keep the local Docker MySQL URL (`mysql://root:password@127.0.0.1:3306/ipollowork_den`) for development only; it is not a production security posture.
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. iPolloWork uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
If you do not deploy Den, worker access is token-based with `IPOLLOWORK_TOKEN` and `IPOLLOWORK_HOST_TOKEN`.

## Minimal Working Stack

#### iPolloWork Cloud and the Desktop App

iPolloWork cloud is composed of two services:
...
iPolloWork does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Skip the server. [**iPolloWork Cloud**](/cloud/get-started) runs hosted workers for your team.
...
If you're rolling iPolloWork out across a company of 150\+ employees, the orchestrator on a single VM might be harder to manage.
