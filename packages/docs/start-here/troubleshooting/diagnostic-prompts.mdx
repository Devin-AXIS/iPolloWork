...
description: "Paste-able prompts that let your agent debug iPolloWalk issues on its own."
---

When something in iPolloWalk looks wrong, the fastest debugger is usually the agent itself: it can read its own configuration, probe servers, and verify fixes in-band. This page collects paste-able diagnostic prompts. Copy the whole block for your situation into a chat and let the agent work through it.
...
Your agent's `ipollowalk-cloud` MCP entry is a snapshot: it stores a server URL and token from the moment it was connected. If it was created against a local development server or an old deployment — including via a leftover `desktop-bootstrap.json` file that overrides server URLs beneath the Settings UI — the agent keeps querying that old world forever, while the dashboard shows the current one.

**Paste this to your agent**

````text
Debug and fix my iPolloWalk Cloud MCP connection. It may be pointed at a stale or local server instead of my real iPolloWalk Cloud server. Work through ALL phases, show your findings after each, and ask before anything destructive. Never print bearer tokens or Authorization headers — redact them.

My expected cloud web URL: https://app.ipollowalklabs.com (replace with your self-hosted Den web URL if you run your own).

## Phase 1 — Evidence: what am I actually connected to?

1. Print the `ipollowalk-cloud` MCP entry's `url` (redact headers) from every config you can find:
...
2. Print these files in full if they exist (they can silently override server URLs beneath the Settings UI):
   - `~/.config/ipollowalk/desktop-bootstrap.json`
   - `~/Library/Application Support/com.differentai.ipollowalk/desktop-bootstrap.json`
...
- ipollowalk-cloud url is localhost/127.0.0.1, OR search returned only gmail-drafts, OR execute returned unknown_capability → you are bound to a STALE server, not the expected cloud.
...
10. Tell me to do the two steps only a human can do, then wait:
    - Restart the iPolloWalk app.
    - Settings → Connections → iPolloWalk Cloud → Disconnect → Connect (rewrites the MCP url to the current server and mints a fresh token).
...
- The `ipollowalk-cloud` entry's `url` points at your real server's `/api/den/mcp/agent`.
...
If the agent completes all phases and verification still fails, [open an issue](https://github.com/Devin-AXIS/iPolloWalk/issues) and paste the agent's final report (it is already redacted).
