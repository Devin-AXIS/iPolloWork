...
description: "Create a GitHub App and configure the iPolloWalk EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted iPolloWalk deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
config:
  public:
    webOrigin: "https://ipollowalk.example.com"
    apiOrigin: "https://api.ipollowalk.example.com"
```

The GitHub App needs these iPolloWalk URLs:

| GitHub App field | Value |
| --- | --- |
| Homepage URL | `https://ipollowalk.example.com` |
| Setup URL | `https://ipollowalk.example.com/dashboard/integrations/github` |
| Webhook URL | `https://api.ipollowalk.example.com/v1/webhooks/connectors/github` |
...
secret:
  create: false
  existingSecret: ipollowalk-ee-secrets
```
...
## What iPolloWalk does with it

When an admin clicks **Connect GitHub** in iPolloWalk:
...
```text
https://ipollowalk.example.com/dashboard/integrations
```

Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to iPolloWalk, show repositories visible to the installation, and allow discovery on a selected repository.
