...
description: "Create a GitHub App and configure the iPolloWork EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted iPolloWork deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
config:
  public:
    webOrigin: "https://ipollowork.example.com"
    apiOrigin: "https://api.ipollowork.example.com"
```

The GitHub App needs these iPolloWork URLs:

| GitHub App field | Value |
| --- | --- |
| Homepage URL | `https://ipollowork.example.com` |
| Setup URL | `https://ipollowork.example.com/dashboard/integrations/github` |
| Webhook URL | `https://api.ipollowork.example.com/v1/webhooks/connectors/github` |
...
secret:
  create: false
  existingSecret: ipollowork-ee-secrets
```
...
## What iPolloWork does with it

When an admin clicks **Connect GitHub** in iPolloWork:
...
```text
https://ipollowork.example.com/dashboard/integrations
```

Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to iPolloWork, show repositories visible to the installation, and allow discovery on a selected repository.
