...
Organization branding lets an owner customize the iPolloWork experience for everyone signed in to an on-prem Den organization. Uploaded images stay inside the deployment: Den stores the binary image data in the existing MySQL database and serves signed, versioned URLs from your Den API origin.
...
<Note>
  Branding changes the displayed organization experience. It does not rename or re-sign the installed executable, and the signed application identity remains iPolloWork.
</Note>
...
config:
  public:
    webOrigin: "https://ipollowork.example.internal"
    apiOrigin: "https://api.ipollowork.example.internal"
    desktopDenBaseUrl: "https://ipollowork.example.internal"
    corsOrigins: "https://ipollowork.example.internal,https://api.ipollowork.example.internal"
    betterAuthTrustedOrigins: "https://ipollowork.example.internal"
    webAppHosts: "ipollowork.example.internal"
```
...
If your organization uses a private certificate authority, install that CA in the operating-system trust store on every desktop. iPolloWork uses Electron networking for the native icon download, so it follows the operating system's proxy and certificate trust.
...
- Use `Clear` and save to return to stock iPolloWork branding.
...
2. Reopen iPolloWork to force an immediate configuration fetch instead of waiting for the hourly background refresh.
