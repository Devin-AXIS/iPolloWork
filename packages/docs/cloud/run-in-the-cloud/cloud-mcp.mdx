---
title: "iPolloWalk Connect MCP"
description: "Use your iPolloWalk organization, skills, and connected tools from Codex, ChatGPT Desktop, OpenCode, Cursor, Claude Code, VS Code, or any MCP client"
---

import { iPolloWalkConnectInstaller } from "/snippets/ipollowalk-connect-installer.jsx";

iPolloWalk Connect lets your own agent use everything available to your iPolloWalk organization. Sign in once to use your shared skills and connected tools, including Gmail, Slack, Notion, and other services your organization has configured.

The server uses OAuth. Your MCP client opens a browser, you sign in to iPolloWalk, choose the organization to authorize, and the client stores the returned token.

<iPolloWalkConnectInstaller />

## In the iPolloWalk desktop app

If you use the iPolloWalk desktop app, there is nothing to configure. When you sign in, the app injects iPolloWalk Connect by default with access scoped to your active organization. Switching organizations refreshes that access automatically.

## What you can ask the agent

Once connected, your agent manages the organization from plain English in the
composer. These flows are exercised end to end against a live iPolloWalk Cloud
deployment (see `evals/cloud-mcp-agent-flows.md` in the repository):

- **Check your cloud identity** — "Which iPolloWalk Cloud organization am I
...
2. Start the client's MCP auth flow for `ipollowalk`.
3. Sign in at `app.ipollowalklabs.com` when the browser opens.
4. Choose the iPolloWalk Cloud organization that should receive MCP access.
...
iPolloWalk Cloud's MCP OAuth flow is hardened for public MCP clients:
...
- JWT access tokens are signed with EdDSA only. The MCP verifier pins that
  signing algorithm, validates the issuer as the iPolloWalk Cloud auth server,
...
- Access is rechecked against the user's active iPolloWalk Cloud session and
  organization membership, so removing a member or revoking their session also
  cuts off MCP access.

## What the server exposes

The hosted MCP server exposes iPolloWalk Cloud API tools for normal product resources, including config objects, connectors, plugins, marketplaces, skills, workers, members, roles, teams, and LLM providers.
...
- An iPolloWalk Cloud account and organization.
...
If your client does not support OAuth for remote MCP servers, it cannot connect to iPolloWalk Connect yet.
