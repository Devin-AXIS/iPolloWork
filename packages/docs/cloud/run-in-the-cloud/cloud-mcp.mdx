---
title: "iPolloWork Connect MCP"
description: "Use your iPolloWork organization, skills, and connected tools from Codex, ChatGPT Desktop, OpenCode, Cursor, Claude Code, VS Code, or any MCP client"
---

import { iPolloWorkConnectInstaller } from "/snippets/ipollowork-connect-installer.jsx";

iPolloWork Connect lets your own agent use everything available to your iPolloWork organization. Sign in once to use your shared skills and connected tools, including Gmail, Slack, Notion, and other services your organization has configured.

The server uses OAuth. Your MCP client opens a browser, you sign in to iPolloWork, choose the organization to authorize, and the client stores the returned token.

<iPolloWorkConnectInstaller />

## In the iPolloWork desktop app

If you use the iPolloWork desktop app, there is nothing to configure. When you sign in, the app injects iPolloWork Connect by default with access scoped to your active organization. Switching organizations refreshes that access automatically.

## What you can ask the agent

Once connected, your agent manages the organization from plain English in the
composer. These flows are exercised end to end against a live iPolloWork Cloud
deployment (see `evals/cloud-mcp-agent-flows.md` in the repository):

- **Check your cloud identity** — "Which iPolloWork Cloud organization am I
...
2. Start the client's MCP auth flow for `ipollowork`.
3. Sign in at `app.ipolloworklabs.com` when the browser opens.
4. Choose the iPolloWork Cloud organization that should receive MCP access.
...
iPolloWork Cloud's MCP OAuth flow is hardened for public MCP clients:
...
- JWT access tokens are signed with EdDSA only. The MCP verifier pins that
  signing algorithm, validates the issuer as the iPolloWork Cloud auth server,
...
- Access is rechecked against the user's active iPolloWork Cloud session and
  organization membership, so removing a member or revoking their session also
  cuts off MCP access.

## What the server exposes

The hosted MCP server exposes iPolloWork Cloud API tools for normal product resources, including config objects, connectors, plugins, marketplaces, skills, workers, members, roles, teams, and LLM providers.
...
- An iPolloWork Cloud account and organization.
...
If your client does not support OAuth for remote MCP servers, it cannot connect to iPolloWork Connect yet.
