---
title: "Get started with iPolloWalk Cloud"
description: "Connect the desktop app to your iPolloWalk Cloud org and unlock shared workspaces, marketplaces, and shared providers"
---

iPolloWalk Cloud requires an active subscription at [app.ipollowalklabs.com](https://app.ipollowalklabs.com/).
It is intended for teams what want to share `SKILLs.md`, `MCPs` and `configs`.


First, you should connect your desktop app with iPolloWalk Cloud.

<Frame>
  ![iPolloWalk Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [app.ipollowalklabs.com](https://app.ipollowalklabs.com/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if iPolloWalk Cloud asks for billing.
...
1. Open the `dashboard/org_settings` [page](https://app.ipollowalklabs.com/dashboard/org-settings) in the cloud app.
...
4. If the browser does not return to iPolloWalk automatically, click `Paste sign-in code`, paste the `ipollowalk://den-auth?...` link or one-time code from iPolloWalk Cloud, then click `Finish sign-in`.

## Cloud URL used by the desktop app

The desktop app uses a single Cloud URL: the Den web `baseUrl`. The hosted
default is `https://app.ipollowalklabs.com`.
...
- iPolloWalk Cloud MCP requests go to `<baseUrl>/api/den/mcp/...`.
...
back to `https://app.ipollowalklabs.com`. When you change the Cloud URL in
`Settings -> Cloud`, iPolloWalk writes the new `baseUrl` back to the bootstrap
...
- Use [Shared workspaces](/cloud/run-in-the-cloud/shared-workspace) to deploy hosted iPolloWalk runtimes that teammates can open from the desktop app.
