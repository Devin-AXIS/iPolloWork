---
title: "Get started with iPolloWork Cloud"
description: "Connect the desktop app to your iPolloWork Cloud org and unlock shared workspaces, marketplaces, and shared providers"
---

iPolloWork Cloud requires an active subscription at [app.ipolloworklabs.com](https://app.ipolloworklabs.com/).
It is intended for teams what want to share `SKILLs.md`, `MCPs` and `configs`.


First, you should connect your desktop app with iPolloWork Cloud.

<Frame>
  ![iPolloWork Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [app.ipolloworklabs.com](https://app.ipolloworklabs.com/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if iPolloWork Cloud asks for billing.
...
1. Open the `dashboard/org_settings` [page](https://app.ipolloworklabs.com/dashboard/org-settings) in the cloud app.
...
4. If the browser does not return to iPolloWork automatically, click `Paste sign-in code`, paste the `ipollowork://den-auth?...` link or one-time code from iPolloWork Cloud, then click `Finish sign-in`.

## Cloud URL used by the desktop app

The desktop app uses a single Cloud URL: the Den web `baseUrl`. The hosted
default is `https://app.ipolloworklabs.com`.
...
- iPolloWork Cloud MCP requests go to `<baseUrl>/api/den/mcp/...`.
...
back to `https://app.ipolloworklabs.com`. When you change the Cloud URL in
`Settings -> Cloud`, iPolloWork writes the new `baseUrl` back to the bootstrap
...
- Use [Shared workspaces](/cloud/run-in-the-cloud/shared-workspace) to deploy hosted iPolloWork runtimes that teammates can open from the desktop app.
