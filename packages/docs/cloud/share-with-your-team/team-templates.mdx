...
description: "Create reusable workspace templates for teams in iPolloWork Cloud"
---
...
1. Build the workspace locally in the iPolloWork desktop app.
2. Add the skills, MCP servers, commands, agents, and providers the team should start with.
3. Share the setup to your iPolloWork Cloud organization.
4. Assign the template to the right team.
5. Ask teammates to open `Template Market` → `Explore`, choose `Cloud` beside `Built-in`, and install the template. Installed Cloud templates are kept in `My templates`.
