...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in iPolloWalk Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in iPolloWalk Cloud

1. Open `MCP Connections` in the iPolloWalk Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in iPolloWalk Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
2. In iPolloWalk Cloud, add the Slack connection and paste the app's client ID
   and client secret.
3. Copy the OAuth redirect URL iPolloWalk shows after creating the connection,
...
4. The browser shows `Connected` — return to iPolloWalk.
...
Your sign-in is stored in iPolloWalk Cloud, not on this machine — connect once
...
The agent discovers the org's tools through the `iPolloWalk Cloud Control`
...
still works, with or without a cloud account. Local servers (like iPolloWalk
