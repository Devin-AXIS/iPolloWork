...
const cli = join(root, "bin", "ipollowork.mjs")
const temp = mkdtempSync(join(tmpdir(), "ipollowork-bootstrap-test-"))
...
try {
  ...
  const executableName = process.platform === "win32" ? "ipollowork-bootstrap.cmd" : "ipollowork-bootstrap"
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      claimLinks: [
        { id: "wcl_owner", role: "owner", token: "test-owner-token", url: "https://app.ipolloworklabs.com/workspace-claim?token=test-owner-token", expiresAt: "2030-01-01T00:00:00.000Z" },
        { id: "wcl_member", role: "member", token: "test-member-token", url: "https://app.ipolloworklabs.com/workspace-claim?token=test-member-token", expiresAt: "2030-01-01T00:00:00.000Z" },
      ],
    }),
    "utf8",
  )
  ...
  assert.equal(claimLinkByRoleJson.claimLinks[0].url, "https://app.ipolloworklabs.com/workspace-claim?token=test-owner-token")
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "skl_test", title: "First iPolloWork Skill", output: "IPOLLOWORK_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  // and `apiBaseUrl`, so a normal `--base-url https://api.ipolloworklabs.com`
  // run broke the desktop app's Sign In flow (it opened
  // `https://api.ipolloworklabs.com/?mode=sign-in...` and showed raw API JSON).
  await withStubDenApi(
    () => ({
      ok: true,
      organization: { id: "org_test", name: "Stub Org", slug: "org_test", status: "provisional" },
