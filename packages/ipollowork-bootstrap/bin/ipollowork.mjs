...
const VERSION = "0.1.0"
// The installed command name. Deliberately NOT "ipollowork" so it never collides
// with the ipollowork-orchestrator npm package, which also installs an "ipollowork"
// binary onto the user's PATH.
const COMMAND_NAME = "ipollowork-bootstrap"
...
function printHelp() {
  console.log([
    "ipollowork-bootstrap",
    "",
    "Usage:",
    "  ipollowork-bootstrap install [--bin-dir <path>] [--install-dir <path>] [--source <path>] [--json]",
    "  ipollowork-bootstrap install app --manifest <url-or-file> [--app-dir <path>] [--json]",
    "  ipollowork-bootstrap doctor [--bin-dir <path>] [--install-dir <path>] [--base-url <url>] [--desktop-bootstrap] [--json]",
    "  IPOLLOWORK_OWNER_PASSWORD=<password> ipollowork-bootstrap cloud onboard --base-url <url> --owner-email <email> --org-name <name> --invite-email <email> [--skill-name <name>] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  ipollowork-bootstrap cloud bootstrap-workspace --base-url <url> --workspace-name <name> [--skill-name <name>] [--owner-email <email>] [--teammate-emails a@x.com,b@y.com] [--claim-roles owner,member] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  ipollowork-bootstrap cloud claim-link [--role owner] [--desktop-bootstrap-path <path>] [--json]",
    "",
    "Commands:",
    "  install          Install the ipollowork-bootstrap CLI into a user bin dir",
    ...
    "                   links). Defaults to https://app.ipolloworklabs.com when",
    "                   --base-url is the hosted API (api.ipolloworklabs.com);",
...
function defaultInstallDir() {
  return process.env.IPOLLOWORK_INSTALL_DIR || join(process.env.HOME || process.cwd(), ".ipollowork", "bootstrap")
}

function defaultBinDir() {
  return process.env.IPOLLOWORK_BIN_DIR || join(process.env.HOME || process.cwd(), ".local", "bin")
}

function defaultAppDir() {
  return process.env.IPOLLOWORK_APP_DIR || (process.platform === "darwin"
    ? join(process.env.HOME || process.cwd(), "Applications")
    : process.platform === "win32"
      ? join(process.env.LOCALAPPDATA || join(process.env.HOME || process.cwd(), "AppData", "Local"), "iPolloWork")
      : join(process.env.HOME || process.cwd(), ".local", "share", "ipollowork"))
}
...
function defaultDesktopBootstrapPath() {
  return process.env.IPOLLOWORK_DESKTOP_BOOTSTRAP_PATH || join(configHomeDir(), "ipollowork", "desktop-bootstrap.json")
}

function defaultSkillsDir() {
  return process.env.IPOLLOWORK_SKILLS_DIR || join(configHomeDir(), "opencode", "skills")
}

function defaultDeviceKeyPath() {
  return process.env.IPOLLOWORK_DEVICE_KEY_PATH || join(configHomeDir(), "ipollowork", "bootstrap-device-key.json")
}
...
// browser opens `https://api.ipolloworklabs.com/?mode=sign-in...` and shows raw
...
function deriveWebBaseUrl(apiBaseUrl) {
  try {
    ...
    if (url.hostname === "api.ipolloworklabs.com") {
      return "https://app.ipolloworklabs.com"
    }
...
function slugifySkillName(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "ipollowork-bootstrap-skill"
}
...
function runInstall(args) {
  ...
  const installedCli = join(installDir, "ipollowork.mjs")
  ...
  jsonOut({ ok: true, message: `iPolloWork CLI installed at ${executable}`, install: manifest }, json)
}
...
function defaultInstalledName(type, manifest, artifact) {
  ...
  if (type === "dmg") return "iPolloWork.app"
  if (type === "appimage") return "iPolloWork.AppImage"
  if (type === "exe") return "iPolloWork.exe"
  if (type === "msi") return "iPolloWork.msi"
  if (process.platform === "darwin") return "iPolloWork.app"
  if (process.platform === "win32") return "iPolloWork.exe"
  return "ipollowork"
}
...
function installDmg(input) {
  ...
  try {
    ...
    mounted = true
    const appName = input.appName || "iPolloWork.app"
...
async function runInstallApp(args) {
  const json = hasFlag(args.flags, "json")
  const manifestLocation = getFlag(args.flags, "manifest") || process.env.IPOLLOWORK_INSTALL_MANIFEST
  ...
  const workDir = join(tmpdir(), `ipollowork-app-install-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  try {
    ...
    const artifactPath = join(workDir, artifact.fileName || "iPolloWork.dmg")
    ...
    writeFileSync(join(appDir, "ipollowork-app-install.json"), JSON.stringify(install, null, 2))
    jsonOut({ ok: true, message: `iPolloWork app installed at ${appPath}`, install }, json)
  } finally {
...
async function runDoctor(args) {
  ...
  checks.push({ name: "ipolloworkExecutable", ok: executableOk, value: executable })
  ...
  if (hasFlag(args.flags, "app") || args.flags.has("app-dir")) {
    const appManifest = join(appDir, "ipollowork-app-install.json")
    let appPath = process.platform === "darwin"
      ? join(appDir, "iPolloWork.app")
      : process.platform === "win32"
        ? join(appDir, "iPolloWork.exe")
        : join(appDir, "ipollowork")
    ...
    checks.push({ name: "ipolloworkApp", ok: existsSync(appPath), value: appPath })
  ...
  jsonOut({ ok, message: ok ? "iPolloWork doctor: ok" : "iPolloWork doctor: failed", version: VERSION, manifest, checks }, json)
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by ipollowork bootstrap.\nipolloworkBootstrapTrigger: bootstrap.verify\nipolloworkBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm iPolloWork cloud onboarding can create and trigger a deterministic skill.`
}
...
function runBootstrapSkill(skill, input) {
  const trigger = readFrontmatterValue(skill.skillText, "ipolloworkBootstrapTrigger")
  const output = readFrontmatterValue(skill.skillText, "ipolloworkBootstrapOutput")
    ...
    ...(input.claimLinks
      ? {
          claimLinks: input.claimLinks.map((link) => ({
            ...
            url: `redacted: run "ipollowork-bootstrap cloud claim-link --role ${link.role}" to view`,
          })),
        }
      : {}),
  }
...
async function resolveOwnerPassword(flags) {
  ...
  const envName = getFlag(flags, "owner-password-env", "IPOLLOWORK_OWNER_PASSWORD")
...
async function runCloudOnboard(args) {
  ...
  const skillName = getFlag(args.flags, "skill-name", "First iPolloWork Skill")
  const skillOutput = getFlag(args.flags, "skill-output", "IPOLLOWORK_BOOTSTRAP_SKILL_TRIGGERED")
  ...
  jsonOut({
    ok: true,
    message: "iPolloWork cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  const json = hasFlag(args.flags, "json")
  const baseUrl = getFlag(args.flags, "base-url", "https://api.ipolloworklabs.com")?.replace(/\/$/, "")
  ...
  const skill = {
    ...response.body.skill,
    skillText: skillText(response.body.skill.title, response.body.skill.output || "IPOLLOWORK_BOOTSTRAP_SKILL_TRIGGERED"),
  }

  const skillRun = runBootstrapSkill(skill, { trigger: "bootstrap.verify" })
  if (!skillRun.triggered || skillRun.output !== "IPOLLOWORK_BOOTSTRAP_SKILL_TRIGGERED") {
    throw new Error(`skill_trigger_failed: ${JSON.stringify(skillRun)}`)
  }
  ...
  jsonOut({
    ok: true,
    message: "iPolloWork workspace bootstrap complete",
    organization: response.body.organization,
    ...
    claimLinks: response.body.claimLinks.map((link) => ({
      ...
      expiresAt: link.expiresAt,
      url: prepareDesktop
        ? `redacted: run "ipollowork-bootstrap cloud claim-link --role ${link.role}" to view`
