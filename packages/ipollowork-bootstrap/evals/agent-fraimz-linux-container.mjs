// Linux e2e fraimz for iPolloWork bootstrap without shipping to production.
//
// This runs inside a real Linux container and proves:
// - the bootstrap CLI installs on Linux,
// - `ipollowork install app` installs a Linux tar.gz app artifact from a manifest,
// - `ipollowork doctor --app` verifies the installed app,
// - `ipollowork cloud onboard` can hit a live Den API from inside Linux.
//
// Required host setup:
//   DEN_API_E2E_BASE_URL=http://host.docker.internal:<den-api-port>
//
// Run from packages/ipollowork-bootstrap:
...
const outDir = join(repoRoot, "evals", "results", "ipollowork-linux-container")
...
const artifactPath = join(fixtureDir, "ipollowork-linux.tar.gz")
...
function redactSecrets(value) {
  return value.replace(/IPOLLOWORK_OWNER_PASSWORD=[^\s]+/g, "IPOLLOWORK_OWNER_PASSWORD=[redacted]")
}
...
try {
  const appBinary = join(artifactRoot, "ipollowork")
  writeFileSync(appBinary, "#!/usr/bin/env sh\necho iPolloWork Linux fixture\n")
  chmodSync(appBinary, 0o755)
  execFileSync("tar", ["-czf", artifactPath, "-C", artifactRoot, "ipollowork"])
  const digest = sha256(artifactPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-linux-fixture",
    artifacts: {
      linux: {
        x64: { type: "tar.gz", url: "file:///fixture/ipollowork-linux.tar.gz", sha256: digest, appName: "ipollowork" },
        arm64: { type: "tar.gz", url: "file:///fixture/ipollowork-linux.tar.gz", sha256: digest, appName: "ipollowork" },
      },
    },
  }, null, 2))
  prove("A Linux app artifact manifest is available without shipping to production", {
    action: "Create local ipollowork tar.gz artifact and manifest with SHA-256",
  ...
  const installAndDoctor = dockerRun([
    "set -euo pipefail",
    "node /package/bin/ipollowork.mjs install --install-dir /tmp/ipollowork/install --bin-dir /tmp/ipollowork/bin --json >/tmp/install.json",
    "/tmp/ipollowork/bin/ipollowork-bootstrap install app --manifest /fixture/install-manifest.json --app-dir /tmp/ipollowork/apps --json >/tmp/app-install.json",
    "/tmp/ipollowork/bin/ipollowork-bootstrap doctor --install-dir /tmp/ipollowork/install --bin-dir /tmp/ipollowork/bin --app --app-dir /tmp/ipollowork/apps --json",
  ].join(" && "), { timeout: 60_000 })
  prove("Linux can install the CLI and app artifact, then doctor the app", {
    action: "Run ipollowork install, ipollowork install app, and ipollowork doctor --app inside Linux",
  ...
  const onboard = dockerRun([
    "set -euo pipefail",
    "node /package/bin/ipollowork.mjs install --install-dir /tmp/ipollowork/install --bin-dir /tmp/ipollowork/bin --json >/tmp/install.json",
    `/tmp/ipollowork/bin/ipollowork-bootstrap cloud onboard --base-url "$DEN_API_E2E_BASE_URL" --owner-email ${JSON.stringify(ownerEmail)} --org-name ${JSON.stringify(`Linux CLI Org ${runId}`)} --invite-email ${JSON.stringify(inviteEmail)} --skill-name ${JSON.stringify(skillName)} --prepare-desktop --desktop-bootstrap-path /tmp/ipollowork/desktop-bootstrap.json --skills-dir /tmp/ipollowork/skills --json >/tmp/onboard.json`,
    "/tmp/ipollowork/bin/ipollowork-bootstrap doctor --install-dir /tmp/ipollowork/install --bin-dir /tmp/ipollowork/bin --desktop-bootstrap --desktop-bootstrap-path /tmp/ipollowork/desktop-bootstrap.json --json >/tmp/doctor.json",
    ...
    env: ["-e", `IPOLLOWORK_OWNER_PASSWORD=${randomPassword()}`],
    timeout: 90_000,
  })
  prove("Linux installed CLI can complete live cloud onboarding", {
    action: "Run ipollowork cloud onboard from inside Linux against live Den API",
    ...
    evidence: onboard,
  }, onboard.status === 0 && onboard.json?.onboard?.ok === true && onboard.json?.onboard?.organization?.id && onboard.json?.onboard?.invitation?.invitationId && onboard.json?.onboard?.skill?.id && onboard.json?.onboard?.skill?.title === skillName && onboard.json?.onboard?.skillRun?.triggered === true && onboard.json?.onboard?.skillRun?.output === "IPOLLOWORK_BOOTSTRAP_SKILL_TRIGGERED" && onboard.json?.onboard?.desktop?.prepared === true && onboard.json?.onboard?.desktop?.bootstrapPath && onboard.json?.onboard?.desktop?.skillPath && onboard.json?.doctor?.ok === true && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrap" && check.ok === true) && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrapHandoff" && check.ok === true))
} finally {
...
<title>iPolloWork Linux Bootstrap — fraimz</title>
...
</head><body><main><h1>iPolloWork Linux Bootstrap — fraimz</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Live Den API: <code>${esc(denBaseUrl)}</code> · Frames: ${frames.length}</div>
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "ipollowork-linux-container",
