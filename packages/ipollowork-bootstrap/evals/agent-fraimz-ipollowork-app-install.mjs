// Live local e2e fraimz for `ipollowork install app` using a real macOS DMG.
//
// The flow creates a tiny iPolloWork.app fixture, packages it as a .dmg, writes an
// install manifest with a SHA-256 digest, installs the bootstrap CLI into a temp
// bin dir, then runs the installed `ipollowork install app` command against the
// manifest and verifies it with `ipollowork doctor --app`.
...
if (process.platform !== "darwin") {
  throw new Error("ipollowork install app DMG fraimz requires macOS")
}

const here = dirname(fileURLToPath(import.meta.url))
const packageRoot = resolve(here, "..")
const repoRoot = resolve(packageRoot, "..", "..")
const cli = join(packageRoot, "bin", "ipollowork.mjs")
const outDir = join(repoRoot, "evals", "results", "ipollowork-app-install-dmg")
const temp = join(tmpdir(), `ipollowork-app-install-dmg-${Date.now()}-${Math.random().toString(36).slice(2)}`)
const sourceDir = join(temp, "source")
const appFixture = join(sourceDir, "iPolloWork.app")
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const appDir = join(temp, "Applications")
const dmgPath = join(temp, "iPolloWork.dmg")
const manifestPath = join(temp, "ipollowork-install-manifest.json")
const installediPolloWork = join(binDir, "ipollowork-bootstrap")
...
<key>CFBundleExecutable</key><string>iPolloWork</string>
<key>CFBundleIdentifier</key><string>com.ipollowork.fixture</string>
<key>CFBundleName</key><string>iPolloWork</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleShortVersionString</key><string>0.0.0-fixture</string>
</dict></plist>
`)
  writeFileSync(join(appFixture, "Contents", "MacOS", "iPolloWork"), "#!/usr/bin/env sh\necho iPolloWork fixture\n")
  chmodSync(join(appFixture, "Contents", "MacOS", "iPolloWork"), 0o755)

  execFileSync("hdiutil", ["create", "-quiet", "-volname", "iPolloWork", "-srcfolder", sourceDir, "-ov", "-format", "UDZO", dmgPath])
  const digest = sha256(dmgPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-fixture",
    appName: "iPolloWork.app",
    artifacts: {
      darwin: {
        [process.arch]: {
          type: "dmg",
  ...
  prove("A real macOS DMG install manifest is available", {
    action: "Create iPolloWork.app fixture, package it with hdiutil, and write manifest JSON",
  ...
  prove("The bootstrap CLI can be installed from a script", {
    action: "node bin/ipollowork.mjs install --install-dir <tmp> --bin-dir <tmp>/bin --json",
    assert: "exit 0 and an ipollowork executable exists",
    evidence: { status: installCli.status, body: installCli.json },
  }, installCli.status === 0 && existsSync(installediPolloWork))

  const installApp = run(installediPolloWork, ["install", "app", "--manifest", manifestPath, "--app-dir", appDir, "--json"], { timeout: 30_000 })
  const appPath = join(appDir, "iPolloWork.app")
  prove("The installed CLI can download, verify, mount, and install iPolloWork.app from a DMG", {
    action: "ipollowork install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
    assert: "exit 0, checksum recorded, and iPolloWork.app copied into app dir",
  ...
  const doctor = run(installediPolloWork, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "ipollowork doctor --app --app-dir <tmp>/Applications --json",
    assert: "exit 0 with ipolloworkApp and appInstallManifest checks passing",
    evidence: { status: doctor.status, body: doctor.json, stderr: doctor.stderr },
  }, doctor.status === 0 && doctor.json?.ok === true && doctor.json?.checks?.some((check) => check.name === "ipolloworkApp" && check.ok) && doctor.json?.checks?.some((check) => check.name === "appInstallManifest" && check.ok))
} finally {
...
<title>iPolloWork App DMG Install — fraimz</title>
...
</head><body><main><h1>iPolloWork App DMG Install — fraimz</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Frames: ${frames.length}</div>
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "ipollowork-app-install-dmg",
