// Live e2e fraimz for the script-installable iPolloWork bootstrap CLI.
//
...
//   DEN_API_E2E_BASE_URL=... node evals/agent-fraimz-ipollowork-bootstrap.mjs
...
const temp = mkdtempSync(join(tmpdir(), "ipollowork-bootstrap-e2e-"))
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const installediPolloWork = join(binDir, process.platform === "win32" ? "ipollowork-bootstrap.cmd" : "ipollowork-bootstrap")
const outDir = join(repoRoot, "evals", "results", "ipollowork-bootstrap-cli")
...
try {
  ...
  prove("A bootstrap script can install the ipollowork CLI", {
  ...
  }, install.status === 0 && install.json?.ok === true && install.json?.install?.executable === installediPolloWork)

  const doctor = run(installediPolloWork, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--base-url", baseUrl, "--json"])
  prove("The installed CLI can doctor itself and the Den API", {
    action: "ipollowork doctor --base-url <live-den-api> --json",
  ...
  const onboard = run(installediPolloWork, [
    "cloud",
    "onboard",
    "--base-url",
    baseUrl,
    ...
    skillName,
    "--json",
  ], { env: { ...process.env, IPOLLOWORK_OWNER_PASSWORD: randomPassword() }, timeout: 30_000 })
  prove("The installed CLI can onboard a user, org, invite, and skill end-to-end", {
    action: "ipollowork cloud onboard --base-url <live-den-api> --owner-email ... --org-name ... --invite-email ... --skill-name ... --json",
  ...
  }, onboard.status === 0 && onboard.json?.ok === true && onboard.json?.organization?.id && onboard.json?.invitation?.invitationId && onboard.json?.skill?.id && onboard.json?.skill?.title === skillName && onboard.json?.skillRun?.triggered === true && onboard.json?.skillRun?.output === "IPOLLOWORK_BOOTSTRAP_SKILL_TRIGGERED")
} finally {
...
<title>iPolloWork Bootstrap CLI — live e2e fraimz</title>
...
<h1>iPolloWork Bootstrap CLI — live e2e fraimz</h1>
