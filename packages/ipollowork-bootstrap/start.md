...
You are an agent helping a user install and set up iPolloWork.

Goal: install iPolloWork, create a provisional workspace without requiring email
...
> The bootstrap command is `ipollowork-bootstrap` (NOT `ipollowork`). The bare
> `ipollowork` command belongs to the separate `ipollowork-orchestrator` package and
> may already be on the user's PATH - do not use it for these steps.

## 1. Install CLI

Install the `ipollowork-bootstrap` CLI into a user-writable bin dir.

If you already have this repository locally:

```bash
node packages/ipollowork-bootstrap/bin/ipollowork.mjs install \
  --install-dir "$HOME/.ipollowork/bootstrap" \
...
`ipollowork-bootstrap` CLI (a single dependency-free Node file) and installs it
into `$HOME/.local/bin` - no npm or npx required.

```bash
curl -fsSLo /tmp/ipollowork-install.sh https://ipolloworklabs.com/install.sh
less /tmp/ipollowork-install.sh
sh /tmp/ipollowork-install.sh
```
...
```bash
ipollowork-bootstrap install app \
  --manifest https://ipolloworklabs.com/install-manifest.json \
  --json
```
...
```bash
ipollowork-bootstrap cloud bootstrap-workspace \
  --base-url https://api.ipolloworklabs.com \
  --workspace-name "<workspace-name>" \
  --skill-name "First iPolloWork Skill" \
  --claim-roles owner \
  --prepare-desktop \
...
1. Confirm in one line that iPolloWork is installed and their workspace is ready
   (use the workspace name).
2. Point them at ONE concrete first task they can run right now, e.g. "iPolloWork
...
```bash
ipollowork-bootstrap cloud claim-link --role owner --json
```
...
- `ipollowork-bootstrap doctor --json` returns `ok: true`
- `ipollowork-bootstrap doctor --app --json` returns `ok: true`
- `ipollowork-bootstrap cloud bootstrap-workspace ... --json` returns:
  - `ok: true`
  - `organization.id` is present
  - `setup.id` is present
  - `skill.id` is present
  - `skillRun.triggered` is `true`
  - `skillRun.output` is `IPOLLOWORK_BOOTSTRAP_SKILL_TRIGGERED`
  - `claimLinks[0].id` is present
  - `desktop.prepared` is `true`
  - `desktop.bootstrapPath` is present
  - `desktop.skillPath` is present
- `ipollowork-bootstrap doctor --desktop-bootstrap --json` returns `ok: true`
...
- If the `ipollowork-bootstrap` command is not found after install: ensure
  `$HOME/.local/bin` is on PATH, or call the binary by its full path
  (`$HOME/.local/bin/ipollowork-bootstrap`).
- If app install fails: run `ipollowork-bootstrap doctor --app --json` and report failed checks.
