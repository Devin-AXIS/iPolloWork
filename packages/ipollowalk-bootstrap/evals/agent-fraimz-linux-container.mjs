// Linux e2e fraimz for iPolloWalk bootstrap without shipping to production.
//
// This runs inside a real Linux container and proves:
// - the bootstrap CLI installs on Linux,
// - `ipollowalk install app` installs a Linux tar.gz app artifact from a manifest,
// - `ipollowalk doctor --app` verifies the installed app,
// - `ipollowalk cloud onboard` can hit a live Den API from inside Linux.
//
// Required host setup:
//   DEN_API_E2E_BASE_URL=http://host.docker.internal:<den-api-port>
//
// Run from packages/ipollowalk-bootstrap:
...
const outDir = join(repoRoot, "evals", "results", "ipollowalk-linux-container")
...
const artifactPath = join(fixtureDir, "ipollowalk-linux.tar.gz")
...
function redactSecrets(value) {
  return value.replace(/IPOLLOWALK_OWNER_PASSWORD=[^\s]+/g, "IPOLLOWALK_OWNER_PASSWORD=[redacted]")
}
...
try {
  const appBinary = join(artifactRoot, "ipollowalk")
  writeFileSync(appBinary, "#!/usr/bin/env sh\necho iPolloWalk Linux fixture\n")
  chmodSync(appBinary, 0o755)
  execFileSync("tar", ["-czf", artifactPath, "-C", artifactRoot, "ipollowalk"])
  const digest = sha256(artifactPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-linux-fixture",
    artifacts: {
      linux: {
        x64: { type: "tar.gz", url: "file:///fixture/ipollowalk-linux.tar.gz", sha256: digest, appName: "ipollowalk" },
        arm64: { type: "tar.gz", url: "file:///fixture/ipollowalk-linux.tar.gz", sha256: digest, appName: "ipollowalk" },
      },
    },
  }, null, 2))
  prove("A Linux app artifact manifest is available without shipping to production", {
    action: "Create local ipollowalk tar.gz artifact and manifest with SHA-256",
  ...
  const installAndDoctor = dockerRun([
    "set -euo pipefail",
    "node /package/bin/ipollowalk.mjs install --install-dir /tmp/ipollowalk/install --bin-dir /tmp/ipollowalk/bin --json >/tmp/install.json",
    "/tmp/ipollowalk/bin/ipollowalk-bootstrap install app --manifest /fixture/install-manifest.json --app-dir /tmp/ipollowalk/apps --json >/tmp/app-install.json",
    "/tmp/ipollowalk/bin/ipollowalk-bootstrap doctor --install-dir /tmp/ipollowalk/install --bin-dir /tmp/ipollowalk/bin --app --app-dir /tmp/ipollowalk/apps --json",
  ].join(" && "), { timeout: 60_000 })
  prove("Linux can install the CLI and app artifact, then doctor the app", {
    action: "Run ipollowalk install, ipollowalk install app, and ipollowalk doctor --app inside Linux",
  ...
  const onboard = dockerRun([
    "set -euo pipefail",
    "node /package/bin/ipollowalk.mjs install --install-dir /tmp/ipollowalk/install --bin-dir /tmp/ipollowalk/bin --json >/tmp/install.json",
    `/tmp/ipollowalk/bin/ipollowalk-bootstrap cloud onboard --base-url "$DEN_API_E2E_BASE_URL" --owner-email ${JSON.stringify(ownerEmail)} --org-name ${JSON.stringify(`Linux CLI Org ${runId}`)} --invite-email ${JSON.stringify(inviteEmail)} --skill-name ${JSON.stringify(skillName)} --prepare-desktop --desktop-bootstrap-path /tmp/ipollowalk/desktop-bootstrap.json --skills-dir /tmp/ipollowalk/skills --json >/tmp/onboard.json`,
    "/tmp/ipollowalk/bin/ipollowalk-bootstrap doctor --install-dir /tmp/ipollowalk/install --bin-dir /tmp/ipollowalk/bin --desktop-bootstrap --desktop-bootstrap-path /tmp/ipollowalk/desktop-bootstrap.json --json >/tmp/doctor.json",
    ...
    env: ["-e", `IPOLLOWALK_OWNER_PASSWORD=${randomPassword()}`],
    timeout: 90_000,
  })
  prove("Linux installed CLI can complete live cloud onboarding", {
    action: "Run ipollowalk cloud onboard from inside Linux against live Den API",
    ...
    evidence: onboard,
  }, onboard.status === 0 && onboard.json?.onboard?.ok === true && onboard.json?.onboard?.organization?.id && onboard.json?.onboard?.invitation?.invitationId && onboard.json?.onboard?.skill?.id && onboard.json?.onboard?.skill?.title === skillName && onboard.json?.onboard?.skillRun?.triggered === true && onboard.json?.onboard?.skillRun?.output === "IPOLLOWALK_BOOTSTRAP_SKILL_TRIGGERED" && onboard.json?.onboard?.desktop?.prepared === true && onboard.json?.onboard?.desktop?.bootstrapPath && onboard.json?.onboard?.desktop?.skillPath && onboard.json?.doctor?.ok === true && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrap" && check.ok === true) && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrapHandoff" && check.ok === true))
} finally {
...
<title>iPolloWalk Linux Bootstrap — fraimz</title>
...
</head><body><main><h1>iPolloWalk Linux Bootstrap — fraimz</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Live Den API: <code>${esc(denBaseUrl)}</code> · Frames: ${frames.length}</div>
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "ipollowalk-linux-container",
