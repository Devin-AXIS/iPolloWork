// Live local e2e fraimz for `ipollowalk install app` using a real macOS DMG.
//
// The flow creates a tiny iPolloWalk.app fixture, packages it as a .dmg, writes an
// install manifest with a SHA-256 digest, installs the bootstrap CLI into a temp
// bin dir, then runs the installed `ipollowalk install app` command against the
// manifest and verifies it with `ipollowalk doctor --app`.
...
if (process.platform !== "darwin") {
  throw new Error("ipollowalk install app DMG fraimz requires macOS")
}

const here = dirname(fileURLToPath(import.meta.url))
const packageRoot = resolve(here, "..")
const repoRoot = resolve(packageRoot, "..", "..")
const cli = join(packageRoot, "bin", "ipollowalk.mjs")
const outDir = join(repoRoot, "evals", "results", "ipollowalk-app-install-dmg")
const temp = join(tmpdir(), `ipollowalk-app-install-dmg-${Date.now()}-${Math.random().toString(36).slice(2)}`)
const sourceDir = join(temp, "source")
const appFixture = join(sourceDir, "iPolloWalk.app")
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const appDir = join(temp, "Applications")
const dmgPath = join(temp, "iPolloWalk.dmg")
const manifestPath = join(temp, "ipollowalk-install-manifest.json")
const installediPolloWalk = join(binDir, "ipollowalk-bootstrap")
...
<key>CFBundleExecutable</key><string>iPolloWalk</string>
<key>CFBundleIdentifier</key><string>com.ipollowalk.fixture</string>
<key>CFBundleName</key><string>iPolloWalk</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleShortVersionString</key><string>0.0.0-fixture</string>
</dict></plist>
`)
  writeFileSync(join(appFixture, "Contents", "MacOS", "iPolloWalk"), "#!/usr/bin/env sh\necho iPolloWalk fixture\n")
  chmodSync(join(appFixture, "Contents", "MacOS", "iPolloWalk"), 0o755)

  execFileSync("hdiutil", ["create", "-quiet", "-volname", "iPolloWalk", "-srcfolder", sourceDir, "-ov", "-format", "UDZO", dmgPath])
  const digest = sha256(dmgPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-fixture",
    appName: "iPolloWalk.app",
    artifacts: {
      darwin: {
        [process.arch]: {
          type: "dmg",
  ...
  prove("A real macOS DMG install manifest is available", {
    action: "Create iPolloWalk.app fixture, package it with hdiutil, and write manifest JSON",
  ...
  prove("The bootstrap CLI can be installed from a script", {
    action: "node bin/ipollowalk.mjs install --install-dir <tmp> --bin-dir <tmp>/bin --json",
    assert: "exit 0 and an ipollowalk executable exists",
    evidence: { status: installCli.status, body: installCli.json },
  }, installCli.status === 0 && existsSync(installediPolloWalk))

  const installApp = run(installediPolloWalk, ["install", "app", "--manifest", manifestPath, "--app-dir", appDir, "--json"], { timeout: 30_000 })
  const appPath = join(appDir, "iPolloWalk.app")
  prove("The installed CLI can download, verify, mount, and install iPolloWalk.app from a DMG", {
    action: "ipollowalk install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
    assert: "exit 0, checksum recorded, and iPolloWalk.app copied into app dir",
  ...
  const doctor = run(installediPolloWalk, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "ipollowalk doctor --app --app-dir <tmp>/Applications --json",
    assert: "exit 0 with ipollowalkApp and appInstallManifest checks passing",
    evidence: { status: doctor.status, body: doctor.json, stderr: doctor.stderr },
  }, doctor.status === 0 && doctor.json?.ok === true && doctor.json?.checks?.some((check) => check.name === "ipollowalkApp" && check.ok) && doctor.json?.checks?.some((check) => check.name === "appInstallManifest" && check.ok))
} finally {
...
<title>iPolloWalk App DMG Install — fraimz</title>
...
</head><body><main><h1>iPolloWalk App DMG Install — fraimz</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Frames: ${frames.length}</div>
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "ipollowalk-app-install-dmg",
