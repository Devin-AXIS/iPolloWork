...
You are an agent helping a user install and set up iPolloWalk.

Goal: install iPolloWalk, create a provisional workspace without requiring email
...
> The bootstrap command is `ipollowalk-bootstrap` (NOT `ipollowalk`). The bare
> `ipollowalk` command belongs to the separate `ipollowalk-orchestrator` package and
> may already be on the user's PATH - do not use it for these steps.

## 1. Install CLI

Install the `ipollowalk-bootstrap` CLI into a user-writable bin dir.

If you already have this repository locally:

```bash
node packages/ipollowalk-bootstrap/bin/ipollowalk.mjs install \
  --install-dir "$HOME/.ipollowalk/bootstrap" \
...
`ipollowalk-bootstrap` CLI (a single dependency-free Node file) and installs it
into `$HOME/.local/bin` - no npm or npx required.

```bash
curl -fsSLo /tmp/ipollowalk-install.sh https://ipollowalklabs.com/install.sh
less /tmp/ipollowalk-install.sh
sh /tmp/ipollowalk-install.sh
```

Verify:

```bash
ipollowalk-bootstrap doctor --json
```
...
```bash
ipollowalk-bootstrap install app \
  --manifest https://ipollowalklabs.com/install-manifest.json \
  --json
```
...
```bash
ipollowalk-bootstrap doctor --app --json
```
...
```bash
ipollowalk-bootstrap cloud bootstrap-workspace \
  --base-url https://api.ipollowalklabs.com \
  --workspace-name "<workspace-name>" \
  --skill-name "First iPolloWalk Skill" \
  --claim-roles owner \
  --prepare-desktop \
...
1. Confirm in one line that iPolloWalk is installed and their workspace is ready
   (use the workspace name).
2. Point them at ONE concrete first task they can run right now, e.g. "iPolloWalk
...
```bash
ipollowalk-bootstrap cloud claim-link --role owner --json
```
...
- `ipollowalk-bootstrap doctor --json` returns `ok: true`
- `ipollowalk-bootstrap doctor --app --json` returns `ok: true`
- `ipollowalk-bootstrap cloud bootstrap-workspace ... --json` returns:
  - `ok: true`
  - `organization.id` is present
  - `setup.id` is present
  - `skill.id` is present
  - `skillRun.triggered` is `true`
  - `skillRun.output` is `IPOLLOWALK_BOOTSTRAP_SKILL_TRIGGERED`
  - `claimLinks[0].id` is present
  - `desktop.prepared` is `true`
  - `desktop.bootstrapPath` is present
  - `desktop.skillPath` is present
- `ipollowalk-bootstrap doctor --desktop-bootstrap --json` returns `ok: true`
...
- If the `ipollowalk-bootstrap` command is not found after install: ensure
  `$HOME/.local/bin` is on PATH, or call the binary by its full path
  (`$HOME/.local/bin/ipollowalk-bootstrap`).
- If app install fails: run `ipollowalk-bootstrap doctor --app --json` and report failed checks.
