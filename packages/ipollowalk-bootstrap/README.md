# iPolloWalk Bootstrap CLI

Script-installable `ipollowalk-bootstrap` command for agent-first onboarding.

This package is intentionally small and does not assume npm is the install
channel. A bootstrap script can place `bin/ipollowalk.mjs` on disk, then run:

```bash
ipollowalk-bootstrap install --bin-dir ~/.local/bin --install-dir ~/.ipollowalk/bootstrap
ipollowalk-bootstrap doctor --json
ipollowalk-bootstrap install app --manifest https://example.com/ipollowalk-install-manifest.json
ipollowalk-bootstrap doctor --app --json
IPOLLOWALK_OWNER_PASSWORD='<generated-password>' ipollowalk-bootstrap cloud onboard --base-url https://den.example.com --owner-email ada@example.com --org-name 'Ada Workspace' --invite-email teammate@example.com --skill-name 'First skill' --json
```
