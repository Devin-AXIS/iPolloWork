...
const VERSION = "0.1.0"
// The installed command name. Deliberately NOT "ipollowalk" so it never collides
// with the ipollowalk-orchestrator npm package, which also installs an "ipollowalk"
// binary onto the user's PATH.
const COMMAND_NAME = "ipollowalk-bootstrap"
...
function printHelp() {
  console.log([
    "ipollowalk-bootstrap",
    "",
    "Usage:",
    "  ipollowalk-bootstrap install [--bin-dir <path>] [--install-dir <path>] [--source <path>] [--json]",
    "  ipollowalk-bootstrap install app --manifest <url-or-file> [--app-dir <path>] [--json]",
    "  ipollowalk-bootstrap doctor [--bin-dir <path>] [--install-dir <path>] [--base-url <url>] [--desktop-bootstrap] [--json]",
    "  IPOLLOWALK_OWNER_PASSWORD=<password> ipollowalk-bootstrap cloud onboard --base-url <url> --owner-email <email> --org-name <name> --invite-email <email> [--skill-name <name>] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  ipollowalk-bootstrap cloud bootstrap-workspace --base-url <url> --workspace-name <name> [--skill-name <name>] [--owner-email <email>] [--teammate-emails a@x.com,b@y.com] [--claim-roles owner,member] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  ipollowalk-bootstrap cloud claim-link [--role owner] [--desktop-bootstrap-path <path>] [--json]",
    "",
    "Commands:",
    "  install          Install the ipollowalk-bootstrap CLI into a user bin dir",
    ...
    "                   links). Defaults to https://app.ipollowalklabs.com when",
    "                   --base-url is the hosted API (api.ipollowalklabs.com);",
...
function defaultInstallDir() {
  return process.env.IPOLLOWALK_INSTALL_DIR || join(process.env.HOME || process.cwd(), ".ipollowalk", "bootstrap")
}

function defaultBinDir() {
  return process.env.IPOLLOWALK_BIN_DIR || join(process.env.HOME || process.cwd(), ".local", "bin")
}

function defaultAppDir() {
  return process.env.IPOLLOWALK_APP_DIR || (process.platform === "darwin"
    ? join(process.env.HOME || process.cwd(), "Applications")
    : process.platform === "win32"
      ? join(process.env.LOCALAPPDATA || join(process.env.HOME || process.cwd(), "AppData", "Local"), "iPolloWalk")
      : join(process.env.HOME || process.cwd(), ".local", "share", "ipollowalk"))
}
...
function defaultDesktopBootstrapPath() {
  return process.env.IPOLLOWALK_DESKTOP_BOOTSTRAP_PATH || join(configHomeDir(), "ipollowalk", "desktop-bootstrap.json")
}

function defaultSkillsDir() {
  return process.env.IPOLLOWALK_SKILLS_DIR || join(configHomeDir(), "opencode", "skills")
}

function defaultDeviceKeyPath() {
  return process.env.IPOLLOWALK_DEVICE_KEY_PATH || join(configHomeDir(), "ipollowalk", "bootstrap-device-key.json")
}
...
// browser opens `https://api.ipollowalklabs.com/?mode=sign-in...` and shows raw
...
function deriveWebBaseUrl(apiBaseUrl) {
  try {
    ...
    if (url.hostname === "api.ipollowalklabs.com") {
      return "https://app.ipollowalklabs.com"
    }
...
function slugifySkillName(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "ipollowalk-bootstrap-skill"
}
...
function runInstall(args) {
  ...
  const installedCli = join(installDir, "ipollowalk.mjs")
  ...
  jsonOut({ ok: true, message: `iPolloWalk CLI installed at ${executable}`, install: manifest }, json)
}
...
function defaultInstalledName(type, manifest, artifact) {
  ...
  if (type === "dmg") return "iPolloWalk.app"
  if (type === "appimage") return "iPolloWalk.AppImage"
  if (type === "exe") return "iPolloWalk.exe"
  if (type === "msi") return "iPolloWalk.msi"
  if (process.platform === "darwin") return "iPolloWalk.app"
  if (process.platform === "win32") return "iPolloWalk.exe"
  return "ipollowalk"
}
...
function installDmg(input) {
  ...
  try {
    ...
    mounted = true
    const appName = input.appName || "iPolloWalk.app"
...
async function runInstallApp(args) {
  const json = hasFlag(args.flags, "json")
  const manifestLocation = getFlag(args.flags, "manifest") || process.env.IPOLLOWALK_INSTALL_MANIFEST
  ...
  const workDir = join(tmpdir(), `ipollowalk-app-install-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  try {
    ...
    const artifactPath = join(workDir, artifact.fileName || "iPolloWalk.dmg")
    ...
    writeFileSync(join(appDir, "ipollowalk-app-install.json"), JSON.stringify(install, null, 2))
    jsonOut({ ok: true, message: `iPolloWalk app installed at ${appPath}`, install }, json)
  } finally {
...
async function runDoctor(args) {
  ...
  checks.push({ name: "ipollowalkExecutable", ok: executableOk, value: executable })
  ...
  if (hasFlag(args.flags, "app") || args.flags.has("app-dir")) {
    const appManifest = join(appDir, "ipollowalk-app-install.json")
    let appPath = process.platform === "darwin"
      ? join(appDir, "iPolloWalk.app")
      : process.platform === "win32"
        ? join(appDir, "iPolloWalk.exe")
        : join(appDir, "ipollowalk")
    ...
    checks.push({ name: "ipollowalkApp", ok: existsSync(appPath), value: appPath })
  ...
  jsonOut({ ok, message: ok ? "iPolloWalk doctor: ok" : "iPolloWalk doctor: failed", version: VERSION, manifest, checks }, json)
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by ipollowalk bootstrap.\nipollowalkBootstrapTrigger: bootstrap.verify\nipollowalkBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm iPolloWalk cloud onboarding can create and trigger a deterministic skill.`
}
...
function runBootstrapSkill(skill, input) {
  const trigger = readFrontmatterValue(skill.skillText, "ipollowalkBootstrapTrigger")
  const output = readFrontmatterValue(skill.skillText, "ipollowalkBootstrapOutput")
    ...
    ...(input.claimLinks
      ? {
          claimLinks: input.claimLinks.map((link) => ({
            ...
            url: `redacted: run "ipollowalk-bootstrap cloud claim-link --role ${link.role}" to view`,
          })),
        }
      : {}),
  }
...
async function resolveOwnerPassword(flags) {
  ...
  const envName = getFlag(flags, "owner-password-env", "IPOLLOWALK_OWNER_PASSWORD")
...
async function runCloudOnboard(args) {
  ...
  const skillName = getFlag(args.flags, "skill-name", "First iPolloWalk Skill")
  const skillOutput = getFlag(args.flags, "skill-output", "IPOLLOWALK_BOOTSTRAP_SKILL_TRIGGERED")
  ...
  jsonOut({
    ok: true,
    message: "iPolloWalk cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  const json = hasFlag(args.flags, "json")
  const baseUrl = getFlag(args.flags, "base-url", "https://api.ipollowalklabs.com")?.replace(/\/$/, "")
  ...
  const skill = {
    ...response.body.skill,
    skillText: skillText(response.body.skill.title, response.body.skill.output || "IPOLLOWALK_BOOTSTRAP_SKILL_TRIGGERED"),
  }

  const skillRun = runBootstrapSkill(skill, { trigger: "bootstrap.verify" })
  if (!skillRun.triggered || skillRun.output !== "IPOLLOWALK_BOOTSTRAP_SKILL_TRIGGERED") {
    throw new Error(`skill_trigger_failed: ${JSON.stringify(skillRun)}`)
  }
  ...
  jsonOut({
    ok: true,
    message: "iPolloWalk workspace bootstrap complete",
    organization: response.body.organization,
    ...
    claimLinks: response.body.claimLinks.map((link) => ({
      ...
      expiresAt: link.expiresAt,
      url: prepareDesktop
        ? `redacted: run "ipollowalk-bootstrap cloud claim-link --role ${link.role}" to view`
