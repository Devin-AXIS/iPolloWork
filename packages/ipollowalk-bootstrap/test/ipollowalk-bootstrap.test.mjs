...
const cli = join(root, "bin", "ipollowalk.mjs")
const temp = mkdtempSync(join(tmpdir(), "ipollowalk-bootstrap-test-"))
...
try {
  ...
  const executableName = process.platform === "win32" ? "ipollowalk-bootstrap.cmd" : "ipollowalk-bootstrap"
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      claimLinks: [
        { id: "wcl_owner", role: "owner", token: "test-owner-token", url: "https://app.ipollowalklabs.com/workspace-claim?token=test-owner-token", expiresAt: "2030-01-01T00:00:00.000Z" },
        { id: "wcl_member", role: "member", token: "test-member-token", url: "https://app.ipollowalklabs.com/workspace-claim?token=test-member-token", expiresAt: "2030-01-01T00:00:00.000Z" },
      ],
    }),
    "utf8",
  )
  ...
  assert.equal(claimLinkByRoleJson.claimLinks[0].url, "https://app.ipollowalklabs.com/workspace-claim?token=test-owner-token")
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "skl_test", title: "First iPolloWalk Skill", output: "IPOLLOWALK_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  // and `apiBaseUrl`, so a normal `--base-url https://api.ipollowalklabs.com`
  // run broke the desktop app's Sign In flow (it opened
  // `https://api.ipollowalklabs.com/?mode=sign-in...` and showed raw API JSON).
  await withStubDenApi(
    () => ({
      ok: true,
      organization: { id: "org_test", name: "Stub Org", slug: "org_test", status: "provisional" },
