...
## 2026-09-08 第四批：Ponytail 整套审计与精简

继续使用 `codex/ponytail-batch-1`，以 `65b528da3` 为本批基线。本批完整执行 `ponytail-audit`、`ponytail`、`ponytail-review` 和 `ponytail-debt`；“完整执行”指检查流程闭环，不表示仓库不存在潜在问题。

### 扫描范围与处理原则

- 全仓 Knip JSON 扫描包含应用、共享包、脚本、验证流程和 vendor，退出 1 表示存在发现。其 1,712 个文件记录不是 1,712 个已确认缺陷。
- 补充 TypeScript AST 与文件重复扫描，共覆盖 2,314 个 JS/TS 文件：apps 947、packages 64、scripts 35、external-plugins 26、vendor 1,242；排除安装依赖、构建产物、生成文件和打包静态资源。
- 逐项核对本批删除符号的全仓引用。保留现用的模型名称显示、工作区状态、路径处理、脱敏以及历史启动偏好清理。
- 桌面依赖通过 `server-packaging.mjs` 装入服务运行时；orchestrator 的平台包供二进制构建使用。overlay 是 Vite 多页面入口；这些静态检查发现均未当作死代码删除。
- vendor 中的独立 registry 资源包需要各自携带运行库，跨包属性查询工具也有公共导出边界。没有为了减少文件数量改写上游包边界。
- 媒体时间线校验需要完整音频清单来发现重复和遗留旁白；当前不能改成找到首个文件就返回，也没有增加缓存或无界并发。

### 实际修改

| 修改 | 行为与验证边界 |
| --- | --- |
| 清理 App 旧工具代码与专用类型 | 删除无调用的模型引用格式化、启动偏好读写、模板头解析、会话列表更新、旧消息分组、工具摘要和附件提取。现用聊天、附件、模型显示实现保留 |
| 合并桌面请求实现 | `desktopFetch` 对远程请求委托 `desktopFetchViaMain`；本地 loopback 仍走原生 fetch，保留流、FormData 和取消语义；空正文状态集合不再每次创建 |
| 合并三份 PPTX 颜色解析 | 使用已有 `pptx-export.ts` owner，兼容导出显式保留透明默认值；普通导出保留深色默认值；RGB 通道统一限制到 0–255 |
| 删除开发脚本的死函数 | 移除无调用的 `canWriteWorkspace` 和唯一为它导入的 `statSync` |
| 修复根测试命令 | `test:orchestrator` 从不存在的 `test:router` 改为实际文件会话测试 `test:files`，包含 CLI 构建与真实服务操作 |
| 隔离文件会话测试 | 服务配置、数据库使用测试临时目录，禁用自动托管引擎；子 CLI 复用当前 Node 路径，消费服务 stdout 防止管道堵塞，失败输出堆栈 |

本批 App 应用源码净减 **853 行**，开发脚本另减 **9 行**。没有新增运行时模块、依赖、路由、数据库表或持久化状态。唯一新增文件为 `evals/flows/orchestrator-files.flow.mjs`，用于独立的 CLI 文件会话验证；复用已有客户端和请求验证流程，没有新建报告目录或技术债文件。

### Ponytail debt 结果

全仓找到 **28 条标记，16 条没有明确升级条件**，全部属于 `vendor/hyperframes`。逐条只读清单已在会话输出；没有把“无升级条件”直接等同于 bug。

已明确的升级条件包括：大型 DOM 采样变慢、关键帧出现独立 duration、解构参数遮蔽导致内联失败、用户需要曲线拟合、SDK 新增保留属性/GSAP 位置操作、Figma 渐变角度探针完成等。缓存活跃性、字符串时间位置和部分回退样式也有已知限制。这些没有通过删除标记或去掉兼容处理来假装解决；本批没有新增技术债标记。

### 验证

| 检查 | 结果 |
| --- | --- |
| App 全量 `bun test --isolate ./tests` | **1,171 pass / 0 fail**，144 个文件、6,024 个断言 |
| 请求边界专项 | **27 pass / 0 fail**；SDK Request、请求覆盖参数、空正文状态、HTTP 错误、二进制及超时参数、原生 loopback 路径 |
| PPTX 三个专项文件 | **50 pass / 0 fail**；颜色默认值、透明度、RGB 边界、可编辑元素及局部图片回退 |
| App 类型检查 | 通过 |
| `pnpm test:orchestrator` | 隔离配置后通过；实际验证目录清单、内容读写、版本冲突拒绝、建目录、重命名、删除、事件和关闭会话 |
| internal fraimz 三个流程 | **3 passed / 0 failed / 0 skipped**；产物 `evals/results/ponytail-batch-4/2026-09-08T06-55-35-133Z/fraimz.html` 已生成 |
| 保留代码逐项比对 | 公共工具文件保留的 28 个声明实现与基线完全相同，删除 35 个无调用声明 |
| 维护性审计 / diff 检查 | 14 个变更文件，0 errors / 0 warnings；`git diff --check` 通过 |
| Node 24 生产构建 | 通过，6m33s；主 chunk `app-DUFA1YHU.js` 8,866.41 kB，gzip 1,845.38 kB。较上一批仅减 1.85 kB，不宣称显著性能提升 |
| 首屏依赖检查 | JSZip 仍为独立动态 chunk，不在 `index.html` 的静态依赖闭包中 |
| 生产页面启动 | 使用当前 worktree 的 Vite preview 与应用内浏览器打开 `/session`，侧栏、项目入口及“请先创建项目”空状态正常渲染；检查结束已关闭预览进程和浏览器页 |
