...
async fn main() -> Result<()> {
    let image = env::var("IPOLLOWORK_MICROSANDBOX_IMAGE")
        .unwrap_or_else(|_| "ipollowork-microsandbox:dev".to_string());
    let name = env::var("IPOLLOWORK_MICROSANDBOX_NAME")
        .unwrap_or_else(|_| "ipollowork-microsandbox-rust".to_string());
    let workspace_dir = env::var("IPOLLOWORK_MICROSANDBOX_WORKSPACE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| default_bind_dir(&name, "workspace"));
    let data_dir = env::var("IPOLLOWORK_MICROSANDBOX_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| default_bind_dir(&name, "data"));
    let replace = env_flag("IPOLLOWORK_MICROSANDBOX_REPLACE");
    let host_port = env::var("IPOLLOWORK_MICROSANDBOX_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(8787);
    let connect_host =
        env::var("IPOLLOWORK_CONNECT_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
    let client_token =
        env::var("IPOLLOWORK_TOKEN").unwrap_or_else(|_| "microsandbox-token".to_string());
    let host_token =
        env::var("IPOLLOWORK_HOST_TOKEN").unwrap_or_else(|_| "microsandbox-host-token".to_string());
    ...
    let mut builder = Sandbox::builder(&name)
        ...
        .memory(2048)
        .cpus(2)
        .env("IPOLLOWORK_CONNECT_HOST", &connect_host)
        .env("IPOLLOWORK_TOKEN", &client_token)
        .env("IPOLLOWORK_HOST_TOKEN", &host_token)
        .env("IPOLLOWORK_APPROVAL_MODE", "auto")
        .port(host_port, 8787)
        .volume("/workspace", |v| {
    ...
    let server = sandbox
        ...
        .await
        .context("failed to start the iPolloWork microsandbox entrypoint inside the VM")?;
...
mod tests {
    ...
    async fn rust_example_smoke_test_checks_health_and_session_endpoints() -> Result<()> {
        let image = env::var("IPOLLOWORK_MICROSANDBOX_IMAGE")
            .unwrap_or_else(|_| "ttl.sh/ipollowork-microsandbox-11559:1d".to_string());
