...
function accountBar(accounts: AccountBinding[], panel: Exclude<Nav, 'accounts'>, primary: AccountBinding | undefined): string {
...
export function renderPublishing(input: StudioPage & { draftId?: string | undefined }): string {
  ...
  if (!account || !data) return shell('发帖', 'publishing', `<div class="simple-page">${heading}${empty('请先接入并启用账号', '每篇草稿和发布记录都属于一个明确的账号。', '<a class="button button-primary" href="/accounts">查看账号</a>')}</div>`)
...
export function renderComments(input: StudioPage & { searchId?: string | undefined; interactions: Interaction[]; reviews: ReviewItem[] }): string {
  ...
  if (!account || !data) return shell('评论', 'comments', `<div class="simple-page">${heading}${empty('请先接入并启用账号', '接入后可搜索、筛选帖子和管理评论。', '<a class="button button-primary" href="/accounts">查看账号</a>')}</div>`)
