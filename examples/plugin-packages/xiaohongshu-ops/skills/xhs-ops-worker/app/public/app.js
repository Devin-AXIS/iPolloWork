(() => {
  ...
  function getHost() {
    hostPromise ??= hostRequest('ui/initialize', { protocolVersion: '2025-11-21', appInfo: { name: '小红书运营台', version: '0.4.16' }, appCapabilities: {} }).then(host => {
  ...
  async function askStudio(kind) {
    let data
    if (draftForm) {
      if (kind === 'draft' && window !== parent) {
        const { id, name, brief, mediaKind, assetIds } = draftInput()
        if (!brief.trim()) throw new Error('请先填写创作要求')
        data = { draftId: id, name, brief, mediaKind, assetIds }
      } else {
        const draft = await saveDraft()
      ...
      } else if (kind === 'resume-search') {
        const settings = Object.fromEntries(new FormData(searchForm))
        if (Object.entries(settings).some(([key, value]) => String(value).trim() !== String(pageData.search[key]))) throw new Error('搜索条件已修改，请点击“搜索帖子”开始新的搜索')
        studioDirty = false
      } else {
        await saveComments()
    ...
    const host = await getHost()
    if (!host.hostContext?.['ai.ipollo/workspace']?.sessionId || !host.hostCapabilities?.message) throw new Error('请先在主软件打开一个可对话的会话，再从右侧「＋」打开小红书运营台。')
    const { prompt, draft, search } = await studioApi('request-action', { kind, ...data })
    if (search) pageData.search = search
    if (draft) {
      pageData.draft = draft
      studioDirty = false
      setLocation('draft', draft.id)
      for (const name of ['title', 'body', 'topics']) draftForm.elements.namedItem(name).value = ''
      updatePreview()
      feedback('旧文案已清空，正在按当前预设和创作要求重新生成…')
    }
    try {
      ...
      if (['search', 'resume-search', 'auto-comment'].includes(kind)) await studioApi('set-search-error', { searchId: pageData.search.id, error: error.message })
      throw error
    }
    feedback(kind === 'draft' ? '正在按当前预设和创作要求重新生成标题、描述和话题，结果会自动回到这里。' : '已交给当前会话执行，结果会自动回到这里。')
    toast('已交给主软件 AI 执行')
    if (['search', 'resume-search', 'auto-comment'].includes(kind)) location.reload()
  }
