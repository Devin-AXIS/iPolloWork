...
export function createMockXhsApp() {
  ...
  app.get('/note/:id', (c) => {
    ...
    const comments = post.comments.length ? post.comments.map((comment) => `<article class="comment" id="${escapeHtml(comment.id)}" data-comment-id="${escapeHtml(comment.id)}"><strong>@${escapeHtml(comment.author)}</strong><p>${escapeHtml(comment.body)}</p>
      ...
      <form method="post" action="/note/${encodeURIComponent(post.id)}/comment/${encodeURIComponent(comment.id)}/reply?viewer=${encodeURIComponent(current.handle)}"><label>回复 @${escapeHtml(comment.author)}<input name="body" required></label><button type="submit">发送给 @${escapeHtml(comment.author)}</button></form></article>`).join('') : '<p id="empty-comments" class="meta">还没有评论</p>'
