...
test('analytics keeps account snapshots and article pages isolated, and rejects invalid imports atomically', async () => {
  ...
  try {
    ...
    assert.match(secondPage, new RegExp(`href="/interactions\\?account=${second.id}"`))
...
test('interaction switching filters reader comments, managed actions and reviews before limiting results', async () => {
  ...
  try {
    ...
    const secondPage = await (await app.request(`/interactions?account=${second.id}`)).text()
    ...
    assert.match(secondPage, new RegExp(`href="/interactions\\?account=${second.id}" aria-current="true"`))
    ...
    for (const panel of ['interactions', 'analytics']) assert.equal((await app.request(`/${panel}?account=missing`)).status, 404)
    db.deleteAccount(second.id)
    for (const panel of ['interactions', 'analytics']) assert.equal((await app.request(`/${panel}?account=${second.id}`)).status, 404)
    db.deleteAccount(first.id)
    assert.doesNotMatch(await (await app.request('/interactions')).text(), /FIRST_READER_COMMENT|SECOND_READER_COMMENT|FIRST_PENDING_REVIEW|SECOND_PENDING_REVIEW|class="account-picker"/)
  } finally { db.close() }
})
