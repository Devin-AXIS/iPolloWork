...
describe("removeElementFromHtml", () => {
  it("removes a self-closing element by id", () => {
    ...
    const { html: updated, matched } = removeElementFromHtml(html, { id: "photo" });
  ...
  it("removes a matched composition host by selector", () => {
    ...
    const { html: updated, matched } = removeElementFromHtml(html, {
  ...
  it("supports fragment html by returning updated body markup", () => {
    ...
    const result = removeElementFromHtml(html, { id: "photo" });

    expect(result.matched).toBe(true);
    expect(result.html).toBe(`<div id="rest"></div>`);
  });

  it("returns matched:false when the target is not found", () => {
    const html = `<div id="photo"></div><div id="rest"></div>`;

    const result = removeElementFromHtml(html, { id: "missing" });
